...
pub unsafe fn parse_mcfg(rsdp_phys: u64) -> Option<EcamInfo> {
    ...
    let mcfg_phys = match mcfg_phys {
        Some(addr) => addr,
        None => {
            crate::kernel::printk::serial_puts("[ACPI] MCFG table not found\n");
            return None;
        }
    };
    ...
    if total_length < entries_offset + entry_size {
        crate::kernel::printk::serial_puts("[ACPI] MCFG table too short\n");
        return None;
    }

    let entry_count = (total_length - entries_offset) / entry_size;
    // SAFETY: entries start at mcfg_phys + entries_offset.
    let entries_ptr = unsafe { (mcfg_hdr as *const u8).add(entries_offset) as *const McfgEntry };
    ...
    crate::kernel::printk::serial_puts("[ACPI] MCFG: no segment group 0 entry\n");
    None
}
