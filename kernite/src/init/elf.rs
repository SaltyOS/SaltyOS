...
pub struct ElfLoadResult {
    ...
    pub brk: u64,
    /// Runtime virtual address of the program header table.
    pub phdr: u64,
    /// Program header entry size.
    pub phent: u64,
    /// Program header count.
    pub phnum: u64,
    /// Runtime virtual address of the first loaded byte.
    pub load_start: u64,
    /// Runtime virtual address just past the loaded image span.
    pub load_end: u64,
}
...
pub fn load_elf(
    ...
    let mut brk: u64 = 0;
    let mut load_start: u64 = u64::MAX;
    ...
    for i in 0..phdr_count {
        ...
        while page_vaddr < seg_end {
            ...
            let frame_phys = if let Some(phys) = vspace.resolve_page(page_vaddr) {
                ...
                let phys = pmm_alloc(&FrameOwner::KernelPrivate {
            ...
            if copy_start < copy_end {
                let src_offset = (copy_start.wrapping_sub(delta).wrapping_sub(phdr.p_vaddr)
    ...
    for i in 0..phdr_count {
        ...
        if phdr.p_type == PT_PHDR {
            phdr_vaddr = phdr.p_vaddr.wrapping_add(delta);
            break;
        }
    }
    if phdr_vaddr == 0 {
        phdr_vaddr = (ehdr.e_phoff).wrapping_add(delta);
    }
...
fn apply_relocations(data: &[u8], delta: u64, vspace: &VSpace) -> Result<(), ElfError> {
    let ehdr = read_struct::<Elf64Ehdr>(data, 0).ok_or(ElfError::TooSmall)?;
    let phdr_base = ehdr.e_phoff as usize;
    let phdr_count = ehdr.e_phnum as usize;
    let phdr_size = ehdr.e_phentsize as usize;
    ...
    let rela_file_offset =
        vaddr_to_file_offset(data, phdr_base, phdr_count, phdr_size, rela_vaddr)?;
    ...
    for i in 0..rela_count {
        ...
        if reloc_type == ELF_RELATIVE_RELOC {
            ...
            let phys = vspace
                .resolve_page(target_page)
                .ok_or(ElfError::RelocFailed)?;
            let ptr = phys_to_virt(phys) as *mut u8;
