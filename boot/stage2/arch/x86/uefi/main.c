...
static uint64_t FindAcpiRsdp(void) {
  ...
  for (i = 0; i < gST->NumberOfTableEntries; i++) {
    if (GuidCompare(&gST->ConfigurationTable[i].VendorGuid, &Acpi20TableGuid) ==
        0) {
  ...
  for (i = 0; i < gST->NumberOfTableEntries; i++) {
    if (GuidCompare(&gST->ConfigurationTable[i].VendorGuid, &AcpiTableGuid) ==
        0) {
      return (uint64_t)(uintptr_t)gST->ConfigurationTable[i].VendorTable;
    }
  }
...
static uint64_t FindSmbios(uint8_t *out_major, uint8_t *out_minor) {
  ...
  for (i = 0; i < gST->NumberOfTableEntries; i++) {
    if (GuidCompare(&gST->ConfigurationTable[i].VendorGuid,
                    &Smbios3TableGuid) == 0) {
  ...
  for (i = 0; i < gST->NumberOfTableEntries; i++) {
    if (GuidCompare(&gST->ConfigurationTable[i].VendorGuid, &SmbiosTableGuid) ==
        0) {
      uint8_t *ep = (uint8_t *)gST->ConfigurationTable[i].VendorTable;
      *out_major = ep[6];
      *out_minor = ep[7];
...
static EFI_STATUS LoadFileAllocated(CHAR16 *Path, uint64_t LoadAddr,
  ...
  status = gBS->HandleProtocol(gImageHandle, &LoadedImageProtocolGuid,
                               (void **)&LoadedImage);
  if (EFI_ERROR(status))
    return status;
  ...
  *FileSize = Size;
  ...
  status =
      gBS->AllocatePages(AllocateAddress, EfiLoaderCode, NumPages, &LoadAddr);
  if (EFI_ERROR(status)) {
    File->Close(File);
    Root->Close(Root);
    return status;
