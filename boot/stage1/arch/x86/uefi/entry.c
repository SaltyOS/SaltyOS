 ...
 */
static EFI_STATUS LoadStage2(EFI_HANDLE ImageHandle, EFI_HANDLE *Stage2Handle) {
  EFI_STATUS status;
  ...
  status = gBS->HandleProtocol(ImageHandle, &LoadedImageProtocolGuid,
                               (void **)&LoadedImage);
  if (EFI_ERROR(status)) {
  ...
  /* Open Stage 2 file */
  status = Root->Open(Root, &Stage2File, Stage2Path, EFI_FILE_MODE_READ, 0);
  if (EFI_ERROR(status)) {
  ...
  status =
      Stage2File->GetInfo(Stage2File, &FileInfoGuid, &FileInfoSize, FileInfo);
  if (EFI_ERROR(status)) {
   ...
   */
  {
    ...
    status = gBS->HandleProtocol(*Stage2Handle, &LoadedImageProtocolGuid,
                                 (void **)&Stage2Li);
    if (EFI_ERROR(status)) {
