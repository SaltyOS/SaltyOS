...
#define BOOTINFO_FLAG_STAGE3_EL2                                               \
  (1 << 4) /* Stage3 entered kernel path from EL2 */
...
#define TLV_NEXT(tlv)                                                          \
  ((struct BootInfoTLV *)((uint8_t *)(tlv) + sizeof(struct BootInfoTLV) +      \
 ...
 */
static inline const struct BootInfoTLV *
bootinfo_find_tlv(const struct BootInfoHeader *hdr, enum BootInfoTLVType type) {
...
static inline void bootinfo_builder_init(struct BootInfoBuilder *builder,
...
static inline bool bootinfo_builder_add_tlv(struct BootInfoBuilder *builder,
