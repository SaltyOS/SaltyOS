...
int elf_calc_size(const void *data, uint64_t *min_vaddr, uint64_t *max_vaddr) {
  ...
  for (uint16_t i = 0; i < ehdr->e_phnum; i++) {
    phdr = (const struct Elf64_Phdr *)((const uint8_t *)data + ehdr->e_phoff +
                                       i * ehdr->e_phentsize);
...
int elf_load(const void *data, size_t data_size, uint64_t phys_addr,
  ...
  for (uint16_t i = 0; i < saved_phnum; i++) {
    phdr = (const struct Elf64_Phdr *)((const uint8_t *)data + saved_phoff +
                                       i * saved_phentsize);
  ...
  err = elf_apply_rela(rela_addr, rela_size, rela_ent, virt_base, delta_phys);
  if (err != ELF_OK) {
    result->error = err;
    return err;
  }
...
static int elf_apply_rela(uint64_t rela_addr, uint64_t rela_size,
   ...
   * The relocation table has been loaded at physical address rela_addr +
   * delta_phys. Relocation targets are written at their physical locations
   * (delta_phys) with values using the virtual base.
   */
  const struct Elf64_Rela *rela =
      (const struct Elf64_Rela *)((uintptr_t)(rela_addr + delta_phys));
