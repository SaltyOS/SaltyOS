...
static EFI_GUID s_FileInfoGuid = {
    0x09576E92,
    0x6D3F,
    0x11D2,
    {0x8E, 0x39, 0x00, 0xA0, 0xC9, 0x69, 0x72, 0x3B}};

static CHAR16 s_KernelPath[] = {'\\', 'E', 'F', 'I',  '\\', 'S', 'A', 'L', 'T',
                                'Y',  'O', 'S', '\\', 'k',  'e', 'r', 'n', 'i',
                                't',  'e', '.', 'e',  'l',  'f', 0};

static CHAR16 s_InitrdPath[] = {'\\', 'E', 'F', 'I', '\\', 'S', 'A', 'L',
                                'T',  'Y', 'O', 'S', '\\', 'i', 'n', 'i',
                                't',  'r', 'd', '.', 'i',  'm', 'g', 0};
...
void stage3_entry_64(struct Stage2Info *info) {
  ...
  if (info && (info->flags & STAGE2_FLAG_HAS_FRAMEBUFFER) &&
    ...
    fb_console_init(info->framebuffer_addr, info->framebuffer_width,
                    info->framebuffer_height, info->framebuffer_pitch,
                    info->framebuffer_bpp, info->fb_red_pos, info->fb_green_pos,
                    info->fb_blue_pos);
  ...
  EFI_SYSTEM_TABLE *systable =
      (EFI_SYSTEM_TABLE *)(uintptr_t)load_u64_unaligned(
  ...
  if (uefi_load_file(bs, image_handle, s_KernelPath, &kernel_buffer,
                     &kernel_file_size) != 0) {
  ...
  if (uefi_load_file(bs, image_handle, s_InitrdPath, &initrd_buffer,
                     &initrd_file_size) == 0) {
  ...
  err = elf_load(kernel_buffer, kernel_file_size, final_load_addr,
                 paging_get_kernel_virt_base(), &load_result);
  if (err != ELF_OK) {
  ...
  uint64_t pt_pool_base = boot_alloc(&ba, PT_POOL_PAGES * 4096, PAGE_SIZE_4K,
  ...
  uint64_t stack_addr =
      boot_alloc(&ba, KERNEL_STACK_SIZE, PAGE_SIZE_4K, BOOT_ALLOC_STACK);
  if (stack_addr == 0) {
  ...
  uint64_t bi_buf =
      boot_alloc(&ba, BOOTINFO_BUFFER_SIZE, PAGE_SIZE_4K, BOOT_ALLOC_BOOTINFO);
  if (bi_buf == 0) {
  ...
  efi_status =
      bs->AllocatePool(EfiLoaderData, memmap_size, (void **)&memmap_buf);
  ...
  efi_status =
      bs->GetMemoryMap(&memmap_size, (EFI_MEMORY_DESCRIPTOR *)memmap_buf,
  ...
  uint64_t pml4 = paging_init_dynamic(pt_pool_base, PT_POOL_PAGES * 4096,
                                      ...
                                      load_result.mem_size, boot_max_addr);
  if (pml4 == 0) {
  ...
  struct BootInfoHeader *bootinfo = handoff_build_bootinfo(
      (void *)(uintptr_t)bi_buf, BOOTINFO_BUFFER_SIZE, info,
      extra_bootinfo_flags, &load_result, (uint64_t)(uintptr_t)initrd_buffer,
      initrd_file_size, ba.records, ba.record_count);
  ...
  __asm__ volatile("mov sp, %0\n\t"
                   ...
                   : "r"((uint64_t)(stack_addr + KERNEL_STACK_SIZE)),
                     "r"((uint64_t)(uintptr_t)bootinfo), "r"(load_result.entry)
                   : "memory");
#else
