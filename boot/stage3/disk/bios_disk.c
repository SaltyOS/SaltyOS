...
/* Bounce buffer for reading above 1MB (BIOS INT 13h needs real-mode addressable memory) */
...
int disk_read(struct DiskDevice *disk, uint64_t lba, uint32_t count, void *buffer)
{
    ...
    if (disk->type == DISK_TYPE_BIOS) {
        ...
        while (remaining > 0) {
            ...
            int result = btx_disk_read(disk->drive_num, cur_lba,
                                       (uint16_t)chunk, (void *)BOUNCE_BUF);
            if (result != 0)
                return DISK_ERR_READ;
...
int disk_read_bytes(struct DiskDevice *disk, uint64_t offset, size_t size, void *buffer)
{
    ...
    if (disk->type == DISK_TYPE_BIOS) {
        ...
        uint64_t end_sector = (offset + size + disk->sector_size - 1) / disk->sector_size;
