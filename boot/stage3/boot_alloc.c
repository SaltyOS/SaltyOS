...
static void record_alloc(struct BootAlloc *ba, uint64_t addr, uint64_t size,
                         uint32_t tag) {
 ...
 */
static uint64_t bios_alloc(struct BootAlloc *ba, uint64_t size,
                           uint64_t align) {
...
static uint64_t uefi_alloc(struct BootAlloc *ba, uint64_t size,
                           uint64_t align) {
  ...
  EFI_STATUS status =
      bs->AllocatePages(AllocateAnyPages, EfiLoaderData, pages, &addr);
  if (EFI_ERROR(status))
    return 0;
...
void boot_alloc_finalize_uefi(struct BootAlloc *ba) { ba->uefi_bs = 0; }
#else
...
void boot_alloc_finalize_uefi(struct BootAlloc *ba) { (void)ba; }
#endif /* STAGE3_UEFI */
