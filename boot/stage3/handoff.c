...
static uint32_t tag_to_memmap_type(uint32_t tag) {
  switch (tag) {
  case BOOT_ALLOC_KERNEL:
    return MEMMAP_KERNEL;
  case BOOT_ALLOC_INITRD:
    return MEMMAP_INITRD;
  case BOOT_ALLOC_BOOTINFO:
    return MEMMAP_BOOTINFO;
  case BOOT_ALLOC_PAGE_TABLES:
    return MEMMAP_BOOTLOADER;
  case BOOT_ALLOC_STACK:
    return MEMMAP_BOOTLOADER;
  case BOOT_ALLOC_GENERIC:
    return MEMMAP_BOOTLOADER;
  default:
    return MEMMAP_BOOTLOADER;
  }
...
struct BootInfoHeader *handoff_build_bootinfo(
  ...
  if (stage2_info->memmap_addr && stage2_info->memmap_count > 0) {
    ...
    entries = (struct BootInfoMemMapEntry *)((uint8_t *)buffer + buffer_size -
                                             entries_size);
    ...
    if (stage2_info->memmap_format == MEMMAP_FORMAT_UEFI) {
      ...
      struct E820Entry *e820 =
          (struct E820Entry *)(uintptr_t)stage2_info->memmap_addr;
    ...
    for (uint32_t i = 0;
         i < alloc_record_count && total < CONFIG_MAX_MEM_REGIONS; i++) {
    ...
    bootinfo_builder_add_tlv(&builder, TLV_MEMMAP, entries,
                             total * sizeof(struct BootInfoMemMapEntry));
  }
  ...
  bootinfo_builder_add_tlv(&builder, TLV_KERNEL_IMAGE, &kernel_info,
                           sizeof(kernel_info));
  ...
  if (initrd_addr != 0 && initrd_size != 0) {
    ...
    bootinfo_builder_add_tlv(&builder, TLV_INITRD, &initrd_info,
                             sizeof(initrd_info));
  }
  ...
  if (stage2_info->framebuffer_addr != 0) {
    ...
    bootinfo_builder_add_tlv(&builder, TLV_FRAMEBUFFER, &fb_info,
                             sizeof(fb_info));
  }

  /* Add ACPI RSDP if present */
  if (stage2_info->rsdp_addr != 0) {
    uint8_t acpi_info[sizeof(struct BootInfoACPI)]
    ...
    bootinfo_builder_add_tlv(&builder, TLV_ACPI_RSDP, &acpi_info,
                             sizeof(acpi_info));
  }

  /* Add SMBIOS if present */
  if (stage2_info->smbios_addr != 0) {
    uint8_t smbios_info[sizeof(struct BootInfoSMBIOS)]
        __attribute__((aligned(8))) = {0};
    memcpy(smbios_info, &stage2_info->smbios_addr,
           sizeof(stage2_info->smbios_addr));
    smbios_info[sizeof(uint64_t)] = stage2_info->smbios_major;
    smbios_info[sizeof(uint64_t) + 1] = stage2_info->smbios_minor;
    bootinfo_builder_add_tlv(&builder, TLV_SMBIOS, &smbios_info,
                             sizeof(smbios_info));
  }
  ...
  bootinfo_builder_add_tlv(&builder, TLV_LOADER_INFO, &loader_info,
                           sizeof(loader_info));
