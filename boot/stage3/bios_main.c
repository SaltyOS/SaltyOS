...
void stage3_entry(struct Stage2Info *info) {
  ...
  if (kernel_file_size <= (FILE_SCRATCH_LIMIT - FILE_SCRATCH_ADDR)) {
    ...
    kernel_load_area =
        boot_alloc(&ba, elf_mem_size, kernel_align, BOOT_ALLOC_KERNEL);
    ...
    uint64_t total_needed =
        ALIGN_UP(kernel_file_size, kernel_align) + elf_mem_size;
    kernel_load_area =
        boot_alloc(&ba, total_needed, kernel_align, BOOT_ALLOC_KERNEL);
    if (kernel_load_area == 0) {
      print_str("Need kernel bytes: ");
  ...
  err =
      elf_load(kernel_buffer, kernel_file_size,
  ...
  uint64_t initrd_addr = boot_alloc(&ba, initrd_entry->size_bytes, PAGE_SIZE_4K,
                                    BOOT_ALLOC_INITRD);
  if (initrd_addr == 0) {
  ...
  ssize_t initrd_loaded = fs_load_from_entry(&g_boot_disk, initrd_entry,
                                             (void *)(uintptr_t)initrd_addr,
                                             initrd_entry->size_bytes);
  if (initrd_loaded < 0) {
  ...
  uint64_t pt_pool =
      boot_alloc(&ba, PT_POOL_SIZE, PAGE_SIZE_4K, BOOT_ALLOC_PAGE_TABLES);
  if (pt_pool == 0) {
  ...
  uint64_t pml4 =
      paging_init_dynamic(pt_pool, PT_POOL_SIZE, g_ctx.kernel_phys_base,
                          g_ctx.kernel_size, PAGING_PAGE_1G);
  if (pml4 == 0) {
  ...
  struct BootInfoHeader *bootinfo = handoff_build_bootinfo(
      (void *)(uintptr_t)bi_buf, BOOTINFO_BUFFER_SIZE, info, 0, &load_result,
      g_ctx.initrd_phys_addr, g_ctx.initrd_size, ba.records, ba.record_count);
  ...
  enter_long_mode_and_jump((uint32_t)pml4, g_ctx.kernel_entry,
