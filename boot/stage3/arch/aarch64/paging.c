...
static void write_pte(uint64_t table_addr, unsigned int index, uint64_t value) {
  volatile uint64_t *entry =
      (volatile uint64_t *)(uintptr_t)(table_addr + index * 8);
  *entry = value;
}
...
static int map_2m_block(uint64_t l0_table, uint64_t virt, uint64_t phys,
                        uint64_t flags) {
...
uint64_t paging_init_dynamic(uint64_t pt_pool_base, uint64_t pt_pool_size,
  ...
  uint64_t kern_flags =
      PTE_SH_IS | PTE_ATTR_IDX(MAIR_IDX_NORMAL_WB) | PTE_AP_RW_EL1;
  ...
  uint64_t addr;
  for (addr = AARCH64_QEMU_VIRT_RAM_BASE; addr < id_limit;
       addr += PAGING_PAGE_2M) {
  ...
  uint64_t map_size =
      (kernel_size + PAGING_PAGE_2M - 1) & ~(PAGING_PAGE_2M - 1);
  for (addr = 0; addr < map_size; addr += PAGING_PAGE_2M) {
    if (map_2m_block(l0, kernel_virt + addr, kernel_phys + addr, kern_flags) !=
        0)
      return 0;
  }

  /* Map UART for early serial (0x0900_0000, 2MB block) */
  uint64_t uart_flags = PTE_SH_NS | PTE_ATTR_IDX(MAIR_IDX_DEVICE_nGnRnE) |
