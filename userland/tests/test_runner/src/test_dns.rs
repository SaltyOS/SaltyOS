...
pub fn run() -> bool {
    ...
    // Test 1: Resolve "google.com" (should succeed via QEMU DNS forwarder)
    {
        ...
        let ip = unsafe { besalt::dns::dns_resolve(b"google.com") };
        if ip != 0 {
    ...
    {
        // SAFETY: IPC context is initialized; dnssrv endpoint at slot 64.
        let ip = unsafe { besalt::dns::dns_resolve(b"nonexistent.invalid") };
        if ip == 0 {
    ...
    {
        // SAFETY: IPC context is initialized; dnssrv endpoint at slot 64.
        let ip1 = unsafe { besalt::dns::dns_resolve(b"example.com") };
        let ip2 = unsafe { besalt::dns::dns_resolve(b"example.com") };
        if ip1 != 0 && ip1 == ip2 {
    ...
    // Test 4: getaddrinfo API
    {
        ...
        let result = unsafe {
            besalt::dns::posix_getaddrinfo(b"example.org\0".as_ptr(), &raw mut info)
        };
    ...
    // Test 5: Reverse DNS lookup
    {
        ...
        if ip != 0 {
            ...
            let len = unsafe {
                besalt::dns::dns_reverse_lookup(ip, hostname.as_mut_ptr(), 128)
            };
            if len > 0 {
                let mut lb = LineBuf::new();
