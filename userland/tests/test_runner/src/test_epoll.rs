...
pub fn run() -> bool {
    ...
    let epfd = unsafe { trona_posix::posix_epoll_create() };
    if epfd < 0 {
    ...
    let ret = unsafe { trona_posix::posix_pipe(pipefds.as_mut_ptr()) };
    if ret != 0 {
    ...
    let ret = unsafe {
        trona_posix::posix_epoll_ctl(epfd, EPOLL_CTL_ADD, read_fd, EPOLLIN, read_fd as u64)
    };
    if ret != 0 {
    ...
    let data = b"hi";
    let written = unsafe { trona_posix::posix_write(write_fd, data.as_ptr(), 2) };
    if written != 2 {
    ...
    let ret = unsafe { trona_posix::posix_epoll_ctl(epfd, EPOLL_CTL_DEL, read_fd, 0, 0) };
    if ret != 0 {
    ...
    unsafe {
        trona_posix::posix_close(read_fd);
        trona_posix::posix_close(write_fd);
        trona_posix::posix_close(epfd);
    }
