...
//!
//! Data flow:
//!   Console IRQ → console server → POSIX_TTYSRV_INPUT_EVENT → line discipline →
...
pub extern "C" fn main(_argc: i32, _argv: *const *const u8, _envp: *const *const u8) -> i32 {
    ...
    if !setup_display_ring() {
        puts(b"[posix_ttysrv] WARN: display ring setup failed, no display output\n");
    }
    ...
    loop {
        ...
        match msg.label {
            ...
            POSIX_TTYSRV_GET_FG_PGRP | POSIX_TTYSRV_SET_FG_PGRP | POSIX_TTYSRV_SET_CTTY | POSIX_TTYSRV_DROP_CTTY => {
        ...
        if skip_reply {
            ...
            if err != 0 {
                puts(b"[posix_ttysrv] reply_recv failed\n");
                break;
            }
        }
    }
