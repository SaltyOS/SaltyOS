...
use crate::types::*;
use crate::input::{pty_has_readable_data, push_slave_byte, refill_slave_ring, signal_vfs_if_readable};
use crate::PTYS;
...
fn signal_vfs_if_master_readable(pty_id: usize, pty: &PtyInstance) {
    if pty_has_master_readable_data(pty) {
...
fn reset_allocated_pty(pty: &mut PtyInstance, active: bool) {
    ...
    pty.master_closed = false;
    pty.slave_closed = false;
}
...
pub unsafe fn handle_pty_alloc(reply: &mut TronaMsg) {
    unsafe {
        for pty_id in 1..MAX_PTYS {
            ...
            if pty.active && (!pty.master_closed || !pty.slave_closed) {
                continue;
            }
...
/// POSIX_TTYSRV_PTY_COLLECT: VFS collects data after notification wake.
...
pub unsafe fn handle_pty_write(msg: &TronaMsg, reply: &mut TronaMsg) {
    unsafe {
        ...
        if pty_id == 0 {
            ...
            for i in 0..count {
                let c = *src.add(i);
                if (pty.termios.c_oflag & OPOST) != 0 && (pty.termios.c_oflag & ONLCR) != 0 && c == b'\n' {
...
pub unsafe fn handle_pty_close(msg: &TronaMsg, reply: &mut TronaMsg) {
    unsafe {
        ...
        if side == PTY_SIDE_MASTER {
            pty.master_closed = true;
        } else {
            pty.slave_closed = true;
        }

        if pty.master_closed && pty.slave_closed && pty_id != 0 {
