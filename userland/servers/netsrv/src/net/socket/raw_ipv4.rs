...
fn push_completion(c: Completion) {
    ...
    unsafe {
        ...
        let slot = (&raw mut COMPLETIONS).cast::<Option<Completion>>().add(head);
...
pub(crate) fn pop_completion() -> Option<Completion> {
    ...
    unsafe {
        ...
        let slot = (&raw mut COMPLETIONS).cast::<Option<Completion>>().add(tail);
...
pub(crate) fn owns_conn_id(conn_id: u32) -> bool {
    conn_id >= CONN_ID_BASE
}
...
pub(crate) fn raw_setsockopt(
    ...
    unsafe {
        let sockets = &raw mut RAW_SOCKETS;
        options::set_option(&mut (*sockets)[idx].opts, SOCK_RAW, level, optname, optval, optlen)
    }
}
...
pub(crate) fn raw_getsockopt(conn_id: u32, level: i32, optname: i32) -> Result<(u64, u32), u64> {
    ...
    unsafe {
        let sockets = &raw mut RAW_SOCKETS;
        let sock = &mut (*sockets)[idx];
        options::get_option(&mut sock.opts, SOCK_RAW, sock.protocol as i32, level, optname)
    }
}
