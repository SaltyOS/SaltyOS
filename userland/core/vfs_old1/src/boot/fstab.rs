...
//! Minimal `/etc/fstab` parser for boot-time root handoff.

use crate::vfs_core::mount;

pub(crate) const MAX_FSTAB_ENTRIES: usize = 16;

pub(crate) const FSTAB_OPTS_MAX: usize = 128;
...
pub(crate) struct FstabEntry {
    ...
    pub(crate) opts: [u8; FSTAB_OPTS_MAX],
...
/// Parse an fstab buffer into a fixed array of entries.
///
/// # Safety
///
...
pub(crate) unsafe fn parse(data: *const u8, len: usize) -> [FstabEntry; MAX_FSTAB_ENTRIES] {
    ...
    while pos < buf.len() && idx < MAX_FSTAB_ENTRIES {
        ...
        let source_cap = entry.source.len();
        let target_cap = entry.target.len();
        let fstype_cap = entry.fstype.len();
        let opts_cap = entry.opts.len();
        entry.active = 1;
        copy_field(
            &mut entry.source,
            &mut entry.source_len,
            fields[0],
            source_cap,
        );
        copy_field(
            &mut entry.target,
            &mut entry.target_len,
            fields[1],
            target_cap,
        );
        copy_field(
            &mut entry.fstype,
            &mut entry.fstype_len,
            fields[2],
            fstype_cap,
        );
        copy_field(&mut entry.opts, &mut entry.opts_len, fields[3], opts_cap);
...
fn split_whitespace<'a>(s: &'a [u8], fields: &mut [&'a [u8]; 6]) -> usize {
    ...
    let mut pos = 0usize;
    while pos < s.len() && count < fields.len() {
...
fn copy_field(dst: &mut [u8], dst_len: &mut u8, src: &[u8], cap: usize) {
    let len = core::cmp::min(src.len(), cap);
    dst[..len].copy_from_slice(&src[..len]);
    *dst_len = len as u8;
}

fn parse_digit(s: &[u8]) -> u8 {
    if let Some(c @ b'0'..=b'9') = s.first().copied() {
        c - b'0'
    } else {
        0
    }
...
fn bytes_match(lhs: &[u8], rhs: &[u8]) -> bool {
    lhs == rhs
}
