...
fn current_timer_value(idx: usize, now_ns: u64) -> (u64, u64, u64, u64) {
    unsafe {
        let p = proctab(idx);
        let remaining_ns = if p.timer_deadline_ns == 0 {
            0
        } else {
            p.timer_deadline_ns.saturating_sub(now_ns)
        };
        let interval_ns = p.timer_interval_ns;
...
pub(crate) fn process_expired_timers() {
    ...
    unsafe {
        for i in 0..proctab_cap() {
            ...
            let _ = super::signal::deliver_signal_to(i, super::PM_SIGALRM);
        }
    }
}
...
pub(crate) unsafe fn handle_setitimer(msg: &TronaMsg, reply: &mut TronaMsg, badge: u64) {
    unsafe {
        ...
        proctab(idx).timer_interval_ns = interval_ns;
        proctab(idx).timer_deadline_ns = if value_ns == 0 {
            0
        } else {
