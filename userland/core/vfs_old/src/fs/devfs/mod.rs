...
pub(crate) struct DevfsVnodeData {
    ...
    pub(crate) mode: u32,
    /// PTY generation captured at `POSIX_TTYSRV_PTY_LOOKUP` time.
    /// Passed back on `POSIX_TTYSRV_PTY_OPEN_SLAVE` so ttysrv can
    /// reject opens against a slot that has been reallocated since
    /// this vdata was created.
...
pub(crate) unsafe fn register() -> VfsResult<()> {
    unsafe { register_fs_type(b"devfs", &raw const DEVFS_VFSOPS, &raw const DEVFS_VOPS) }
}
