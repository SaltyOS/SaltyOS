...
unsafe fn sysctlfs_unmount(ctx: &mut OwnerMountCtx<'_>, _force: bool) -> VfsResult<()> {
    unsafe {
...
unsafe fn sysctlfs_root(ctx: &OwnerMountCtx<'_>) -> VfsResult<VnodeHandle> {
    unsafe {
...
unsafe fn sysctlfs_vget(_ctx: &mut OwnerMountCtx<'_>, _id: u64) -> VfsResult<VnodeHandle> {
    Err(VfsError::NotFound)
}

unsafe fn sysctlfs_statfs(_ctx: &OwnerMountCtx<'_>, out: *mut VStatfs) -> VfsResult<()> {
    unsafe {
        (*out).bsize = 4096;
        (*out).blocks = 0;
        (*out).bfree = 0;
...
unsafe fn sysctlfs_sync(_ctx: &OwnerMountCtx<'_>) -> VfsResult<()> {
    Ok(())
}
