...
/// Maximum length of a `DynamicName` entry name.
pub(crate) const DYNAMIC_NAME_MAX: usize = 32;

/// A single entry name produced by `DynamicDir.list`.
#[repr(C)]
#[derive(Clone, Copy)]
pub(crate) struct DynamicName {
    pub(crate) bytes: [u8; DYNAMIC_NAME_MAX],
    pub(crate) len: u8,
}
...
///
/// `list` fills `out` with all child names, returns the count written.
/// `lookup` generates the content for the named child into `buf`, returns
/// bytes written, or `None` if the child does not exist.
///
/// Both callbacks are `unsafe` because they may perform raw IPC and are
/// always called from an unsafe context by the VFS layer.
#[repr(C)]
pub(crate) struct DynamicDir {
    pub(crate) name: [u8; NODE_NAME_MAX],
    pub(crate) name_len: u8,
    _pad: [u8; 7],
    pub(crate) list: unsafe fn(out: *mut DynamicName, out_cap: usize) -> usize,
    pub(crate) lookup:
        unsafe fn(name: *const u8, name_len: usize, buf: *mut u8, buf_cap: usize) -> Option<usize>,
}
...
impl DynamicDir {
    ...
    /// tree. Both `list` and `lookup` are called from an unsafe context.
    pub(crate) const fn new(
        name: &[u8],
        list: unsafe fn(*mut DynamicName, usize) -> usize,
        lookup: unsafe fn(*const u8, usize, *mut u8, usize) -> Option<usize>,
    ) -> Self {
