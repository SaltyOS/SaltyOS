...
pub(crate) unsafe fn handle_ioctl(
    ...
    unsafe {
        ...
        let (kind, device) = match state.open_object_at(cli_handle, fd as usize) {
            Some(obj) => (obj.kind(), obj.device_info()),
            None => {
                (*reply).label = TRONA_INVALID_ARGUMENT;
                return;
            }
        ...
        if kind == ObjectKind::InetSocket && is_net_ioctl(request) {
            ...
            let err = ipc::call_ctx(ipc_ctx(), netsrv_ep(), &raw const nreq, &raw mut nreply);
        ...
        match request {
            ...
            0x5410 => {
                ...
                if !procmgr_call1(INIT_GETSID_BADGE, badge, &raw mut caller_sid) {
                    (*reply).label = TRONA_INVALID_OPERATION;
                    return;
                }
                let mut treq = TronaMsg::zeroed();
                ...
                if (*reply).label == TRONA_OK {
                    let _ = procmgr_call3(INIT_SET_SESSION_TTY_PGRP, badge, (*msg).regs[2], 0);
                }
            }
            ...
            0x540E => {
                ...
                if !procmgr_call1(INIT_GETSID_BADGE, badge, &raw mut caller_sid)
                    || !procmgr_call1(INIT_GETPGID_BADGE, badge, &raw mut caller_pgid)
                {
                    (*reply).label = TRONA_INVALID_OPERATION;
                    return;
                }
                ...
                (*reply).length = 0;
                if (*reply).label == TRONA_OK {
                    let _ = procmgr_call3(INIT_SET_SESSION_TTY, badge, tty_dev, caller_pgid);
                }
            }
            ...
            0x5422 => {
                ...
                (*reply).length = 0;
                if (*reply).label == TRONA_OK {
                    let _ = procmgr_call3(INIT_CLEAR_SESSION_TTY, badge, 0, 0);
                }
            }
