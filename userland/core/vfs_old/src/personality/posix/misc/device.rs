...
pub(crate) unsafe fn handle_device_read(
    ...
    unsafe {
        let device = if fd >= 0 && (fd as usize) < MAX_CLIENT_OBJECTS {
            state
                .open_object_at(cli_handle, fd as usize)
                .and_then(|obj| obj.device_info())
        } else {
            None
        };
...
unsafe fn handle_ptmx_read(pty_id: u32, msg: *const TronaMsg, reply: *mut TronaMsg) {
    unsafe {
...
unsafe fn handle_pty_write(dev_type: u8, pty_id: u32, msg: *const TronaMsg, reply: *mut TronaMsg) {
    unsafe {
        let count = ((*msg).regs[1]).min(152);
