...
pub(super) unsafe fn fetch_pty_termios(pty_id: u64, termios: *mut Termios) -> bool {
    unsafe {
        ...
        let err = ipc::call_ctx(ipc_ctx(), posix_ttysrv_ep(), &raw const req, &raw mut reply);
...
unsafe fn send_pty_timeout_reply(state: &mut VfsState, op: crate::owner::op::OpCore) {
    unsafe {
        let mut wake = TronaMsg::zeroed();
        wake.label = TRONA_OK;
        wake.length = 1;
        wake.regs[0] = 0;
        state.complete_op(op, OwnerPostOp::None, &raw const wake);
    }
}
...
pub(crate) unsafe fn expire_pty_read_timeouts(state: &mut VfsState, now_ns: u64) -> bool {
    ...
    for pty_id in 0..MAX_PTYS {
        ...
        while index < state.pty_pending_count[pty_id] {
            ...
            unsafe {
                send_pty_timeout_reply(state, reader.op);
            }
...
pub(crate) unsafe fn next_pty_read_timeout_ns(state: &VfsState, now_ns: u64) -> u64 {
...
pub(crate) unsafe fn handle_pty_timer(state: &mut VfsState, cookie: u32, now_ns: u64) {
    unsafe {
        if cookie != PTY_TIMER_COOKIE.load(Ordering::Acquire) {
            return;
        }
        PTY_TIMER_DEADLINE.store(0, Ordering::Release);
        let _ = expire_pty_read_timeouts(state, now_ns);
        ...
        for pty_id in 0..MAX_PTYS {
            ...
            for idx in 0..count {
                ...
                if reader.active != 0 && reader.badge == badge {
                    send_client_exit_error(state, reader.op.reply_slot);
                    cleared = true;
                    continue;
                }
                if keep != idx {
...
pub(crate) unsafe fn handle_pty_dev_read(
    ...
    unsafe {
        ...
        let (pty_id, nonblocking) = match state.open_object_at(cli_handle, fd as usize) {
            Some(obj) => (
                obj.device_info().map(|d| d.pty_id).unwrap_or(0),
                obj.nonblocking != 0,
            ),
            None => {
        ...
        let op = match state.begin_op_for_client(cli_handle, OpKind::PtyRead) {
            Ok(op) => op,
            Err(err) => {
...
pub(crate) unsafe fn handle_pty_notification(state: &mut VfsState, ntfn_badge: u64) {
    unsafe {
        for pty_id in 0..MAX_PTYS {
            ...
            while state.pty_pending_count[pty_id] > 0 {
                ...
                state.complete_op(reader.op, OwnerPostOp::None, &raw const wake);
                ...
                state.pty_pending_count[pty_id] -= 1;
                state.pty_pending[pty_id][state.pty_pending_count[pty_id]] =
            ...
            super::super::poll::wake_pty_poll_waiters(state, pty_id as u32);
        }
    }
}
