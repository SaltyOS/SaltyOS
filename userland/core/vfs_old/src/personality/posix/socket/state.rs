...
pub(crate) fn alloc_socket(state: &mut VfsState) -> Option<Handle<SocketState>> {
    ...
    for index in 0..socket.pending_cap as usize {
        unsafe {
            *socket.pending.add(index) = PendingConn::zeroed();
        }
    }
    ...
    socket.data_head = 0;
    socket.data_tail = 0;
    socket.accept_op = OpCore::INVALID;
    socket.accept_badge = 0;
    socket.recv_op = OpCore::INVALID;
    socket.recv_badge = 0;
...
pub(crate) unsafe fn find_socket(
    state: &VfsState,
    handle: Handle<SocketState>,
) -> *mut SocketState {
    unsafe {
        state
            .sockets
