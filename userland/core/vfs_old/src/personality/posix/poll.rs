...
use crate::personality::posix::types::{
    EpollEntry, EpollInstance, PipeState, PollWaiter, SocketState,
};
    ...
    loop {
            ...
            .is_ok()
        {
            timer_wheel::register_timer(
                deadline_ns,
                TimerKind::PollDeadline,
                next_poll_timer_cookie(),
            );
            return;
        }
    }
...
fn check_fd_readiness(state: &VfsState, cli_handle: ClientHandle, fd: i32, events: u32) -> u32 {
    ...
    match slot.kind() {
        ...
        ObjectKind::InetSocket => unsafe {
            ...
            req.regs[1] = events as u64;
            req.length = 2;
            let err = ipc::call_ctx(ipc_ctx(), netsrv_ep(), &raw const req, &raw mut rep);
            if err == 0 && rep.label == TRONA_OK {
                revents = rep.regs[0] as u32;
            }
        },
...
fn try_deliver_ready_waiter(
    state: &mut VfsState,
    cli_handle: ClientHandle,
    waiter_handle: Handle<PollWaiter>,
) -> bool {
    let (kind, op, nfds, objects, data) = match state.poll_waiters.get(waiter_handle) {
    ...
    match kind {
        POLL_WAITER_KIND_POLL => {
            ...
            for i in 0..nfds as usize {
                let rev =
                    check_fd_readiness(state, cli_handle, objects[i].0, objects[i].1 as u32) as u16;
    ...
    unsafe { state.complete_op(op, OwnerPostOp::None, &raw const reply) };
    release_waiter(state, waiter_handle);
    true
}

pub(crate) fn next_poll_deadline_ns(state: &VfsState) -> u64 {
...
pub(crate) fn next_poll_timeout_ns(state: &VfsState, now_ns: u64) -> u64 {
    let deadline = next_poll_deadline_ns(state);
    if deadline == 0 {
        0
...
pub(crate) unsafe fn expire_poll_timeouts(state: &mut VfsState, now_ns: u64) -> bool {
    ...
    loop {
        ...
        for handle in batch.into_iter().take(count) {
            let op = match state.poll_waiters.get(handle) {
                Some(waiter) if waiter.active != 0 => waiter.op,
                _ => continue,
            };
            let mut reply = TronaMsg::zeroed();
            reply.label = TRONA_OK;
...
fn rearm_poll_deadline_from_state(state: &VfsState) {
    let next_deadline = next_poll_deadline_ns(state);
    if next_deadline != 0 {
        arm_poll_deadline(next_deadline);
    }
}

pub(crate) unsafe fn handle_poll_timer(state: &mut VfsState, cookie: u32, now_ns: u64) {
    unsafe {
        if cookie != POLL_TIMER_COOKIE.load(Ordering::Acquire) {
            return;
        }
        POLL_TIMER_DEADLINE.store(0, Ordering::Release);
        let _ = expire_poll_timeouts(state, now_ns);
        rearm_poll_deadline_from_state(state);
    }
}

pub(crate) unsafe fn wake_poll_waiters(state: &mut VfsState, badge: u64, fd: i32, revents: u16) {
    loop {
        let mut batch = [Handle::<PollWaiter>::INVALID; MAX_WAITER_BATCH];
        let mut count = 0usize;
        state.poll_waiters.for_each_active(|handle, waiter| {
        ...
        for handle in batch.into_iter().take(count) {
            ...
            unsafe { state.complete_op(waiter.op, OwnerPostOp::None, &raw const wake_reply) };
            release_waiter(state, handle);
...
/// hang forever.
pub(crate) unsafe fn wake_pty_poll_waiters(state: &mut VfsState, pty_id: u32) {
    loop {
        let mut batch = [Handle::<PollWaiter>::INVALID; MAX_WAITER_BATCH];
        let mut count = 0usize;
        state.poll_waiters.for_each_active(|handle, waiter| {
        ...
        for handle in batch.into_iter().take(count) {
            ...
            for j in 0..waiter.nfds as usize {
                ...
                let matches_pty = match state
                    .open_object_at(cli_handle, fd as usize)
                    .and_then(|o| o.device_info())
                {
...
pub(crate) unsafe fn handle_epoll_wait(
    ...
    unsafe {
        ...
        for i in 0..entries_cap {
            ...
            let rev = check_fd_readiness(
                state,
                cli_handle,
                (*entries.add(i)).fd,
                (*entries.add(i)).events,
            );
            if rev != 0 {
                (*reply).regs[1 + ready_count * 2] = rev as u64;
        ...
        let badge = state
            ...
            .map(|cli| cli.badge)
            .unwrap_or(0);
        let op = match state.begin_op_for_client(cli_handle, OpKind::EpollWait) {
            Ok(op) => op,
            Err(err) => {
        ...
        let Some(waiter_handle) = state.poll_waiters.alloc() else {
            ...
            unsafe { state.complete_op(op, OwnerPostOp::None, &raw const err_reply) };
            return true;
        };
...
pub(crate) unsafe fn handle_poll(
    ...
    unsafe {
        ...
        let badge = state
            ...
            .map(|cli| cli.badge)
            .unwrap_or(0);
        let op = match state.begin_op_for_client(cli_handle, OpKind::Poll) {
            Ok(op) => op,
            Err(err) => {
                (*reply).label = err.to_trona();
                return false;
