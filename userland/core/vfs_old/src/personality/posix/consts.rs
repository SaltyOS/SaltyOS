...
pub(crate) const DEV_URANDOM: u8 = trona_posix::consts::DEV_URANDOM;
...
// `SOCK_BUF_SIZE` lives in `crate::ipc_objects` as the authoritative
// sizing choice for the neutral socket backing; re-exported here so
// existing `use personality::posix::consts::*` callers continue to
// compile unchanged.
pub(crate) use crate::ipc_objects::SOCK_BUF_SIZE;
...
// Pipe limits and pool sizes. `PIPE_BUF_SIZE` lives in
// `crate::ipc_objects` alongside the neutral `PipeState` backing.
pub(crate) use crate::ipc_objects::PIPE_BUF_SIZE;
