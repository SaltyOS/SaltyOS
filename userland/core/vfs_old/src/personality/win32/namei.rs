...
use crate::vfs_core::namei_common::{
    ...
    NAMEI_SYMLINK_MAX_DEPTH, NameiArgs, NameiCtx, NameiResult, cross_covered_personality,
    finish_namei, lookup_component, lookup_component_ci, mount_nosymfollow, readlink_vnode,
    vnode_vtype, walk_dotdot,
};
...
pub(crate) fn namei_win32(ctx: &mut NameiCtx<'_>, args: &NameiArgs) -> VfsResult<NameiResult> {
...
fn lookup_pipefs_root(ctx: &mut NameiCtx<'_>, root_vh: VnodeHandle) -> Option<VnodeHandle> {
    if !root_vh.is_valid() {
        return None;
    }
