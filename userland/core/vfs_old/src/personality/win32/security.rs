...
use crate::vfs_core::vop_context::OwnerVopCtx;
...
pub(crate) unsafe fn check_nt_acl(
    ...
    let sd_len = unsafe { get_security_descriptor(ctx, sd_buf.as_mut_ptr(), sd_buf.len())? };
...
pub(crate) unsafe fn set_security_descriptor(
    ...
    unsafe {
        ...
        match ((*ops).data.setxattr)(
            ...
            Ok(Ready(())) => Ok(()),
            Ok(Parked(_)) => Err(VfsError::WouldBlock),
            Err(e) => Err(e),
        }
    }
}
