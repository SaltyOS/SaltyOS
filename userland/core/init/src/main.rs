...
use child_layout::{COFF_CNODE, COFF_EP};
...
// Init-local notifications allocated at boot. Keep them above selftest scratch
// slots and below the pre-created endpoint pool.
const CAP_ROOTFS_READY_NTFN: u64 = 137;
const CAP_PTY_NTFN: u64 = 138;
...
/// Map init's own cspace slot number (as seeded by `kernite/src/init.rs`
/// during kernel bootstrap) to the system `ROLE_*` identifier and default
/// cap_table flags the slot represents, if any.
///
/// Used by `boot_services` when building `ExtraCapCopy` entries from
/// `CopyCap=src:dst` declarations: if `src` names one of init's
/// role-bearing slots (COM1 ioport, PCI ioport, initrd/fb untypeds), a
/// matching `ROLE_*` entry is also pushed into the child's startup
/// cap_table so the child can reach the cap via `trona::caps::*()`.
///
/// Slots that do not back a role (IRQ handlers, notifications, internal
/// scratch) return `None` and propagate only as a plain CopyCap.
fn init_slot_to_role(init_slot: u64) -> Option<(u32, u32)> {
    use trona::consts::kernel::{
        CAP_TBL_FLAG_DEVICE_UT, CAP_TBL_FLAG_IO_PORT, CAP_TBL_FLAG_UNTYPED, ROLE_COM1_IOPORT,
        ROLE_FB_UNTYPED, ROLE_INITRD_UNTYPED, ROLE_PCI_IOPORT,
    };
    // Slot numbers match kernite/src/init.rs's CAP_* assignments for the
    // init task. See `CLAUDE.md` "Well-Known Capability Slots" table.
    match init_slot {
        8 => Some((ROLE_COM1_IOPORT, CAP_TBL_FLAG_IO_PORT)), // CAP_COM1_IOPORT
        12 => Some((
            ROLE_INITRD_UNTYPED,
        ...
        15 => Some((ROLE_PCI_IOPORT, CAP_TBL_FLAG_IO_PORT)), // CAP_PCI_IOPORT
        _ => None,
    }
}

const PM_WAIT_ANY_CHILD: u64 = u32::MAX as u64;
const ROOTFS_TARGET_READY_BITS: u64 = 1;
const ROOTFS_TARGET_FAILED_BITS: u64 = 1 << 1;
...
// All other role-bearing AT_TRONA_*_EP / AT_TRONA_*_NTFN / AT_TRONA_*_UNTYPED
// / AT_TRONA_*_IOPORT tags are delivered exclusively through the role-based
// startup cap_table (AT_TRONA_CAP_TABLE). Only the structural / bootstrap
// tags above remain on the legacy path.
    ...
    unsafe {
        ...
        for (index, def) in defs.iter().enumerate() {
            ...
            let err = if bootstrap_authority_ep != 0 {
                ...
                invoke::untyped_retype(ut, trona::OBJ_ENDPOINT, 0, ep_slot)
            };
            if err != 0 {
                trona::uerror!(|_lb| {
...
unsafe fn read_boot_info() -> (u64, usize) {
    unsafe {
        ...
        if magic != BOOTINFO_MAGIC {
            trona::uerror!(|_lb| {
                _lb.str(b"[INIT] WARN: boot info magic mismatch\n");
            });
...
unsafe fn init_bootinfo_snapshot_frame(ut: Cap) -> bool {
    unsafe {
        ...
        if err != 0 {
            trona::uerror!(|_lb| {
                _lb.str(b"[INIT] FAIL: bootinfo snapshot frame retype\n");
            });
            return false;
        }
        ...
        if err != 0 {
            trona::uerror!(|_lb| {
                _lb.str(b"[INIT] FAIL: bootinfo snapshot scratch map\n");
            });
            return false;
        }
...
/// Best-effort `PM_KILL` send. Drops any reply / error — used for the
/// abandon-on-failure paths where the caller has nothing to do with the
/// outcome other than continue cleanup.
unsafe fn pm_kill_best_effort(pm_ep: Cap, pid: u32, signo: i32) {
    unsafe {
        if pm_ep == 0 || pid == 0 {
            return;
        }
        ...
        msg.regs[1] = signo as u64;
        let mut reply = TronaMsg::zeroed();
        let _ = ipc::call_ctx(ipc_ctx(), pm_ep, &raw const msg, &raw mut reply);
    }
}

/// Tear down a service that failed during startup: SIGKILL its procmgr
/// child (if any), reap any resulting zombie, and mark the service Failed.
/// Safe to call even when the spawn never reached procmgr — both the
/// SIGKILL and the zombie reap are best-effort no-ops in that case.
unsafe fn cleanup_failed_service_start(
    ...
    unsafe {
        pm_kill_best_effort(pm_ep, mgr.services[svc_idx].pid, SIGKILL);
        mgr.services[svc_idx].pid = 0;
        mgr.set_state(svc_idx, svc_mgr::ServiceState::Failed);
        let _ = drain_zombies(mgr, pm_ep, bootstrap_authority_ep);
    }
}

unsafe fn inject_needep_cap(
    consumer_name: &[u8],
    provider_name: &[u8],
    procmgr_ep: Cap,
    pid: u32,
    ...
    unsafe {
        if procmgr_ep == 0 || provider_ep == 0 {
            return false;
        }

        trona::uinfo!(|_lb| {
            _lb.str(if late {
                b"[INIT] inject late NeedEP "
            } else {
                b"[INIT] inject NeedEP "
            });
            _lb.bytes(consumer_name);
            _lb.str(b" <- ");
        ...
        let r = spawn::pm_inject_cap(procmgr_ep, pid, dst_slot, provider_ep, badge);
        if r != 0 {
            trona::uerror!(|_lb| {
                _lb.str(if late {
                    b"[INIT] WARN: inject late NeedEP "
                } else {
                    b"[INIT] WARN: inject NeedEP "
                });
                _lb.bytes(provider_name);
                _lb.str(b" slot=");
                _lb.hex(dst_slot);
...
unsafe fn inject_needep_cap_init_spawned(
    consumer_name: &[u8],
    provider_name: &[u8],
    consumer_cap_base: u64,
    dst_slot: u64,
    provider_ep: Cap,
    badge: u64,
) -> bool {
    unsafe {
        if consumer_cap_base == 0 || provider_ep == 0 {
            return false;
        }

        let child_cn = consumer_cap_base + child_layout::COFF_CNODE;
        if child_cn == 0 {
            return false;
        }

        trona::uinfo!(|_lb| {
            _lb.str(b"[INIT] inject late NeedEP ");
            _lb.bytes(consumer_name);
            _lb.str(b" <- ");
            _lb.bytes(provider_name);
            _lb.str(b" slot=");
        ...
        let r = if badge != 0 {
            invoke::cnode_mint(CAP_SELF_CSPACE, provider_ep, child_cn, dst_slot, badge)
        } else {
            invoke::cnode_copy(
                CAP_SELF_CSPACE,
                provider_ep,
        ...
        if r != 0 {
            trona::uerror!(|_lb| {
                _lb.str(b"[INIT] WARN: inject late NeedEP ");
                _lb.bytes(provider_name);
                _lb.str(b" slot=");
                _lb.hex(dst_slot);
                _lb.str(b" failed\n");
...
unsafe fn mark_service_running(mgr: &mut svc_mgr::ServiceManager, svc_idx: usize, procmgr_ep: Cap) {
    unsafe {
        mgr.set_state(svc_idx, svc_mgr::ServiceState::Running);

        let provider_ep = mgr.services[svc_idx].pre_ep;
        if provider_ep == 0 {
            return;
        }

        deliver_late_needeps_for_provider(
            mgr,
            procmgr_ep,
            mgr.services[svc_idx].def.name_bytes(),
            provider_ep,
        );
    }
}
...
unsafe fn inject_caps_and_resume(
    ...
    unsafe {
        ...
        for i in 0..ep_need_count as usize {
            ...
            if provider_idx >= 0 {
                let provider = &mgr.services[provider_idx as usize];
                let provider_ep = provider.pre_ep;
                if provider_ep != 0 {
                    if ep_needs[i].late && provider.state != svc_mgr::ServiceState::Running {
                        continue;
                    }
                    let _ = inject_needep_cap(
                        name,
                        svc_name,
                        procmgr_ep,
                        pid,
                        ep_needs[i].dst_slot,
                        provider_ep,
                        if ep_needs[i].badged { pid as u64 } else { 0 },
                        ep_needs[i].late,
                    );
                }
            }
        ...
        for i in 0..cap_count as usize {
            ...
            let r = spawn::pm_inject_cap(procmgr_ep, pid, caps[i].dst_slot, caps[i].src_slot, 0);
            if r != 0 {
                trona::uerror!(|_lb| {
                    _lb.str(b"[INIT] WARN: inject CopyCap svc=");
                    _lb.bytes(name);
                    _lb.str(b" pid=");
                    _lb.hex(pid as u64);
                    _lb.str(b" src=");
        ...
        for i in 0..create_ep_count as usize {
            ...
            let r = spawn::pm_inject_cap(procmgr_ep, pid, create_eps[i].dst_slot, cap, 0);
            if r != 0 {
                trona::uerror!(|_lb| {
                ...
                let _ = invoke::cnode_delete(CAP_SELF_CSPACE, cap);
            }
        }

        // Child was spawned suspended; resume only after all cap injections.
        trona::uinfo!(|_lb| {
            _lb.str(b"[INIT] pm_resume call ");
            _lb.bytes(name);
            _lb.str(b" pid=");
            _lb.dec(pid as u64);
            _lb.str(b"\n");
...
unsafe fn deliver_late_needeps_for_provider(
    mgr: &svc_mgr::ServiceManager,
    procmgr_ep: Cap,
    provider_name: &[u8],
    provider_ep: Cap,
    ...
    unsafe {
        ...
        for i in 0..mgr.count {
            let consumer = &mgr.services[i];
            if consumer.state != svc_mgr::ServiceState::Running {
                continue;
            }

            for j in 0..consumer.def.ep_need_count as usize {
                let need = consumer.def.ep_needs[j];
                if !need.late {
                    continue;
                }

                let need_provider = &need.service[..need.service_len as usize];
                if !bytes_eq(need_provider, provider_name) {
                    continue;
                }

                let badge = if need.badged { consumer.pid as u64 } else { 0 };
                if consumer.cap_base != 0 {
                    let _ = inject_needep_cap_init_spawned(
                        consumer.def.name_bytes(),
                        provider_name,
                        consumer.cap_base,
                        need.dst_slot,
                        provider_ep,
...
unsafe fn load_service_defs(mgr: &mut svc_mgr::ServiceManager) {
    trona::uinfo!(|_lb| {
        _lb.str(b"[INIT] Loading service definitions...\n");
    });
    ...
    unsafe {
        ...
        while cpio::cpio_next(initrd, initrd_size, &raw mut offset, &raw mut entry) != 0 {
            // Check if file path starts with "services/" and uses a known unit suffix.
            let name = core::slice::from_raw_parts(entry.name, entry.name_len);

            if !starts_with(name, b"services/")
                || !(ends_with(name, b".service") || ends_with(name, b".target"))
            {
                continue;
            }

            trona::udebug!(|_lb| {
                _lb.str(b"[INIT] Found unit file: ");
                _lb.bytes(name);
                _lb.str(b"\n");
            });
            ...
            if ini::parse_service(data, &mut def) {
                trona::udebug!(|_lb| {
                    _lb.str(b"[INIT] Parsed service: ");
                    _lb.bytes(def.name_bytes());
                    _lb.str(b" binary=");
                    _lb.bytes(def.binary_bytes());
                    _lb.str(b"\n");
                });
                mgr.add_service(&def);
            } else {
                trona::uerror!(|_lb| {
                    _lb.str(b"[INIT] WARN: failed to parse unit file\n");
                });
            }
        }

        trona::uinfo!(|_lb| {
            _lb.str(b"[INIT] Found ");
            _lb.hex(mgr.count as u64);
            _lb.str(b" services\n");
        });
    }
}
...
unsafe fn poll_rootfs_target_events(rootfs_ready_ntfn: Cap) -> u64 {
    unsafe {
        if rootfs_ready_ntfn == 0 {
            return 0;
        }

        let poll = syscall(SYS_POLL, rootfs_ready_ntfn, 0, 0, 0, 0, 0);
        if poll.error == 0 {
            poll.value
            ...
            trona::uerror!(|_lb| {
                _lb.str(b"[INIT] rootfs target poll failed err=");
                _lb.hex(poll.error);
                _lb.str(b"\n");
            });
            0
...
unsafe fn handle_rootfs_target_events(
    mgr: &mut svc_mgr::ServiceManager,
    rootfs_ready_ntfn: Cap,
) -> bool {
    unsafe {
        let target_events = poll_rootfs_target_events(rootfs_ready_ntfn);
        if (target_events & ROOTFS_TARGET_READY_BITS) != 0 {
            if mgr.reach_event_targets() {
                trona::uinfo!(|_lb| {
                    _lb.str(b"[INIT] event target reached\n");
                });
            }
            return true;
        }
        if (target_events & ROOTFS_TARGET_FAILED_BITS) != 0 {
            if mgr.fail_event_targets() {
                trona::uerror!(|_lb| {
                    _lb.str(b"[INIT] event target failed\n");
                });
            }
            return true;
        }
...
unsafe fn pre_create_endpoints(mgr: &mut svc_mgr::ServiceManager, ut: Cap) {
    trona::uinfo!(|_lb| {
        _lb.str(b"[INIT] Pre-creating service endpoints...\n");
    });
    for i in 0..mgr.count {
...
unsafe fn boot_services(
    ...
    // Set once after init has shipped the parsed post-procmgr service defs
    // to procmgr via PM_REGISTER_SERVICE_DEFS. Guards the lazy call site
    // below so the registry is transferred exactly once.
    let mut service_defs_registered: bool = false;
    let mut procmgr_raw_ep: Cap = 0; // Raw (unbadged) EP for minting into children
                                     // Captured by SpawnRole::Authority spawns. The system has exactly one
                                     // bootstrap-authority service (rsrcsrv); these caps are init's view of
                                     // its endpoint, kept generic so adding a second authority later does
                                     // not require renaming half of init.
    let mut bootstrap_authority_ep: Cap = 0; // INIT_BADGE-minted EP for init's own RES_* calls
    let mut bootstrap_authority_raw_ep: Cap = 0; // Raw (unbadged) EP for minting into children
    loop {
        let mut made_progress = false;

        // Auto-promote passive targets whose deps are all Running
        if mgr.promote_passive_targets() {
            made_progress = true;
        }
        ...
        for order_idx in 0..mgr.boot_order_len {
            ...
            let svc_state = mgr.services[svc_idx].state;
            if svc_state == svc_mgr::ServiceState::Running
                || svc_state == svc_mgr::ServiceState::Failed
            {
                continue;
            }
            if svc_state == svc_mgr::ServiceState::Starting
                && mgr.services[svc_idx].def.svc_type != ini::ServiceType::Target
            {
                continue;
            }

            if mgr.deps_failed(svc_idx) {
                trona::uerror!(|_lb| {
                    _lb.str(b"[INIT] Dependencies failed for ");
                    _lb.bytes(mgr.services[svc_idx].def.name_bytes());
                    _lb.str(b", marking Failed\n");
                });
                mgr.set_state(svc_idx, svc_mgr::ServiceState::Failed);
                made_progress = true;
                continue;
            ...
            if mgr.services[svc_idx].def.svc_type == ini::ServiceType::Target {
                if mgr.services[svc_idx].def.target_activation == ini::TargetActivation::Passive {
                    if mgr.services[svc_idx].state != svc_mgr::ServiceState::Starting {
            ...
            if is_pre_procmgr {
                ...
                const MAX_EXTRA_CAPS: usize =
                    ini::MAX_CAP_COPIES + ini::MAX_EP_NEEDS + ini::MAX_CREATE_EPS;
                ...
                if !create_declared_endpoints(
                    ...
                    cleanup_failed_service_start(mgr, svc_idx, procmgr_ep, bootstrap_authority_ep);
                    pre_spawn_idx += 1;
                    made_progress = true;
                    continue;
                }
                ...
                for i in 0..cap_count as usize {
                    if n < extras.len() {
                        // CopyCap=src:dst entries copy from init's own cspace.
                        // If `src` names an init slot that backs a known system
                        // role (com1_ioport, pci_ioport, initrd_untyped,
                        // fb_untyped), also emit the matching cap_table entry
                        // so the child can read it via `trona::caps::*()`.
                        let (role_id, role_flags) =
                            init_slot_to_role(caps[i].src_slot).unwrap_or((0, 0));
                ...
                for i in 0..ep_need_count as usize {
                    ...
                    if provider_idx >= 0 && n < extras.len() {
                        ...
                        if pre_ep != 0 {
                            let provider_role =
                                mgr.services[provider_idx as usize].def.role_bytes();
                            ...
                            // If the NeedEP provider is a known system role
                            // name (namesrv, vfs, mmsrv, procmgr, console,
                            // rsrcsrv, ...), also push the matching cap_table
                            // role entry so the child can reach the cap via
                            // `trona::caps::<name>()` instead of hardcoding
                            // the NeedEP-assigned slot. For mmsrv/rsrcsrv,
                            // unbadged entries resolve to the raw-authority
                            // role variant (procmgr-private, not exposed via
                            // `trona::caps::*`).
                            let (role_id, role_flags) =
                                ini::system_role_for_needep(svc_name, ep_needs[i].badged)
                                    .unwrap_or((0, 0));
                            extras[n] = ExtraCapCopy {
                                src: if ep_needs[i].late { 0 } else { pre_ep },
                ...
                let spawn_role = if bytes_eq(svc_role, b"authority") {
                    spawn::SpawnRole::Authority
                } else if bytes_eq(svc_role, b"pager") {
                    spawn::SpawnRole::Pager
                } else {
                    spawn::SpawnRole::Service
                };
                let copy_shared_lib_caps = mgr.services[svc_idx].def.copy_shared_lib_caps;
                let mut registered_pid: u32 = 0;
                let err = unsafe {
                    spawn::spawn_server(
                        ut,
                        cap_base,
                ...
                mgr.services[svc_idx].pid = registered_pid;
                ...
                if bytes_eq(name, b"procmgr") {
                    ...
                    unsafe {
                        ...
                        if merr == 0 {
                            ...
                            trona::udebug!(|_lb| {
                                _lb.str(b"[INIT] Minted badged PM EP (badge=1)\n");
                            });
                        } else {
                            procmgr_ep = ep;
                            trona::uerror!(|_lb| {
                                _lb.str(b"[INIT] WARN: mint badged PM EP failed\n");
                            });
                        }
                    }
                }

                // Authority-class spawn: capture the bootstrap authority's
                // EP for init's own future RES_* calls (INIT_BADGE-minted)
                // and for child auto-mint (raw / unbadged). enum-driven,
                // not name-matched.
                if spawn_role == spawn::SpawnRole::Authority {
                    let ep = cap_base + COFF_EP;
                    bootstrap_authority_raw_ep = ep;
                    unsafe {
                        let badged = INIT_DYN_FRAME_NEXT;
                        INIT_DYN_FRAME_NEXT += 1;
                        let merr = invoke::cnode_mint(
                            CAP_SELF_CSPACE,
                            ep,
                        ...
                        if merr == 0 {
                            bootstrap_authority_ep = badged;
                            trona::udebug!(|_lb| {
                                _lb.str(b"[INIT] Minted INIT_BADGE bootstrap-authority EP\n");
                            });
                        } else {
                            bootstrap_authority_ep = ep;
                            trona::uerror!(|_lb| {
                                _lb.str(b"[INIT] WARN: mint INIT_BADGE bootstrap-authority EP failed err=");
                                _lb.hex(merr as u64);
                                _lb.str(b"\n");
                            });
                        }
                ...
                // Service whose .service file declares BootstrapPrivileged=yes:
                // promote its spawn badge to the bootstrap authority's
                // privileged caller set so it can allocate kernel objects on
                // behalf of arbitrary owners.
                if mgr.services[svc_idx].def.bootstrap_privileged && bootstrap_authority_ep != 0 {
                    let mut req = TronaMsg::zeroed();
                    req.label = RES_SET_QUOTA;
                    req.length = 3;
                    req.regs[0] = child_badge;
                    req.regs[1] = 0;
                    req.regs[2] = RES_QUOTA_FLAG_PROMOTE_PRIVILEGED;
                    let mut resp = TronaMsg::zeroed();
                    let perr = unsafe {
                        ipc::call_ctx(
                            ipc_ctx(),
                    ...
                    if perr == 0 && resp.label == TRONA_OK {
                        trona::uinfo!(|_lb| {
                            _lb.str(b"[INIT] promoted ");
                            _lb.bytes(name);
                            _lb.str(b" badge=");
                            _lb.hex(child_badge);
                            _lb.str(b" to bootstrap-authority privileged set\n");
                        });
                    } else {
                        trona::uerror!(|_lb| {
                            _lb.str(b"[INIT] WARN: promote ");
                            _lb.bytes(name);
                            _lb.str(b" to bootstrap-authority privileged failed err=");
                            _lb.hex(perr as u64);
                            _lb.str(b" label=");
                            _lb.hex(resp.label);
                            _lb.str(b"\n");
                ...
                pre_spawn_idx += 1;
                mark_service_running(mgr, svc_idx, procmgr_ep);
                made_progress = true;
                ...
                if procmgr_ep == 0 {
                    trona::uerror!(|_lb| {
                        _lb.str(b"[INIT] Cannot spawn ");
                        _lb.bytes(name);
                        _lb.str(b" - procmgr not available\n");
                    });
                    mgr.set_state(svc_idx, svc_mgr::ServiceState::Failed);
                    made_progress = true;
                    continue;
                ...
                // Lazy one-shot: ship the parsed post-procmgr service defs
                // to procmgr the first time we are about to invoke pm_spawn,
                // then register every pre-procmgr provider so procmgr can
                // resolve `Require=<provider>` for post-procmgr children
                // whose providers are pre-procmgr services.
                //
                // procmgr is now known to be Running and ready to handle
                // IPC; both calls must complete before any post-procmgr
                // child gets its cap_table built.
                if !service_defs_registered {
                    let r = unsafe {
                        spawn::pm_register_service_defs(procmgr_ep, bootstrap_authority_ep, mgr)
                    };
                    if r != 0 {
                        trona::uerror!(|_lb| {
                            _lb.str(b"[INIT] pm_register_service_defs failed rc=");
                            _lb.dec(if r >= 0 { r as u64 } else { (-r) as u64 });
                            _lb.str(b"\n");
                        });
                        mgr.set_state(svc_idx, svc_mgr::ServiceState::Failed);
                        made_progress = true;
                    ...
                    let _ = unsafe { spawn::pm_register_pre_procmgr_providers(procmgr_ep, mgr) };
                    service_defs_registered = true;
                }

                let spawn_name = elf_name;
                trona::uinfo!(|_lb| {
                    _lb.str(b"[INIT] pm_spawn call ");
                    _lb.bytes(name);
                    _lb.str(b"\n");
                });
                let pid = unsafe {
                ...
                trona::uinfo!(|_lb| {
                    _lb.str(b"[INIT] pm_spawn return ");
                    _lb.bytes(name);
                    if pid >= 0 {
                        _lb.str(b" pid=");
                        _lb.dec(pid as u64);
                ...
                trona::uinfo!(|_lb| {
                    _lb.str(b"[INIT] inject/resume call ");
                    _lb.bytes(name);
                    _lb.str(b" pid=");
                    _lb.dec(pid as u64);
                    _lb.str(b"\n");
                ...
                trona::uinfo!(|_lb| {
                    _lb.str(b"[INIT] inject/resume ok ");
                    _lb.bytes(name);
                    _lb.str(b" pid=");
                    _lb.dec(pid as u64);
                    _lb.str(b"\n");
        ...
        if unsafe { handle_rootfs_target_events(mgr, rootfs_ready_ntfn) } {
            continue;
        }

        if !mgr.has_pending_units() {
            break;
        }
        ...
        if unsafe { drain_zombies(mgr, procmgr_ep, bootstrap_authority_ep) } {
            continue;
        }
    ...
    (procmgr_ep, bootstrap_authority_ep)
}
...
unsafe fn handle_child_exit(
    ...
    trona::uinfo!(|_lb| {
        _lb.str(b"[INIT] Child exited: pid=");
        _lb.hex(child_pid as u64);
        _lb.str(b" status=");
        _lb.hex(exit_status as u64);
        _lb.str(b"\n");
    });
    ...
    if idx < 0 {
        trona::udebug!(|_lb| {
            _lb.str(b"[INIT] Unknown child pid, ignoring\n");
        });
        return;
    }
    ...
    if mgr.should_restart(svc_idx) {
        unsafe {
            ...
            if new_pid < 0 {
                trona::uerror!(|_lb| {
                    _lb.str(b"[INIT] Failed to restart service\n");
                });
                mgr.set_state(svc_idx, svc_mgr::ServiceState::Failed);
            } else if !inject_caps_and_resume(
                mgr,
                svc_idx,
                pm_ep,
                new_pid as u32,
                ...
                cleanup_failed_service_start(mgr, svc_idx, pm_ep, bootstrap_authority_ep);
            } else {
                mgr.services[svc_idx].pid = new_pid as u32;
                mark_service_running(mgr, svc_idx, pm_ep);
            }
        }
    }
}
...
unsafe fn drain_zombies(
    ...
    unsafe {
        ...
        loop {
            ...
            let exit_status = reply.regs[0] as i32;
            handle_child_exit(mgr, pm_ep, bootstrap_authority_ep, child_pid, exit_status);
            handled_any = true;
        }
    }
}
...
unsafe fn service_monitor(
    ...
    loop {
        ...
        unsafe { handle_child_exit(mgr, pm_ep, bootstrap_authority_ep, child_pid, exit_status) };
        unsafe { drain_zombies(mgr, pm_ep, bootstrap_authority_ep) };
    }
}
...
pub extern "C" fn _start() -> ! {
    trona::uinfo!(|_lb| {
        _lb.str(b"[INIT] SaltyOS init process starting\n");
    });

    let ut: Cap = select_init_work_untyped();
    trona::udebug!(|_lb| {
        _lb.str(b"[INIT] Bootstrap untyped slot=");
        _lb.hex(ut);
        _lb.str(b"\n");
    });
    ...
    if err != 0 {
        trona::uerror!(|_lb| {
            _lb.str(b"[INIT] FAIL: IPC buf frame retype\n");
        });
        idle();
    }
    let err = invoke::vspace_map(
    ...
    trona::udebug!(|_lb| {
        _lb.str(b"[INIT] IPC buffer mapped at ");
        _lb.hex(IPC_BUF_VADDR);
        _lb.str(b"\n");
    });
    ...
    trona::udebug!(|_lb| {
        _lb.str(b"[INIT] Initrd size from boot info: ");
    ...
    trona::uinfo!(|_lb| {
        _lb.str(b"[INIT] Total usable RAM: ");
        _lb.hex(total_usable);
        _lb.str(b" bytes\n");
    });
    ...
    if run_selftest {
        trona::uinfo!(|_lb| {
            _lb.str(b"[INIT] Self-test mode enabled\n");
        });
    ...
    if mgr.count == 0 {
        trona::uerror!(|_lb| {
            _lb.str(b"[INIT] No service definitions found in initrd\n");
        });
        idle();
    }
    ...
    if !sort_ok {
        trona::uerror!(|_lb| {
            _lb.str(b"[INIT] WARNING: dependency cycle detected, some services may not start\n");
        });
    }
    mgr.log_boot_order();
    ...
    // Exactly one service must declare Role=authority and exactly one must
    // declare Role=pager. The authority is init's last direct-retype path
    // (rsrcsrv), the pager is init's source for child VM fault handlers
    // (mmsrv); both are load-bearing for every subsequent spawn.
    {
        let mut authority_count = 0u32;
        let mut pager_count = 0u32;
        for i in 0..mgr.count {
            let role = mgr.services[i].def.role_bytes();
            if bytes_eq(role, b"authority") {
                authority_count += 1;
            } else if bytes_eq(role, b"pager") {
                pager_count += 1;
            }
        }
        if authority_count != 1 {
            trona::uerror!(|_lb| {
                _lb.str(b"[INIT] FATAL: exactly one service must declare Role=authority, found ");
                _lb.hex(authority_count as u64);
                _lb.str(b"\n");
            });
            idle();
        }
    ...
    // Create dedicated rootfs-ready notification for VFS→init target promotion
    {
        let err = invoke::untyped_retype(ut, OBJ_NOTIFICATION, 0, CAP_ROOTFS_READY_NTFN);
        if err != 0 {
            trona::uerror!(|_lb| {
                _lb.str(b"[INIT] FATAL: rootfs target notification retype failed err=");
                _lb.hex(err as u64);
                _lb.str(b"\n");
            });
            idle();
    ...
    // Create shared PTY notification for VFS↔TTYD data-ready signalling
    {
        ...
        if err != 0 {
            ...
            trona::udebug!(|_lb| {
                _lb.str(b"[INIT] PTY notification created at slot 14\n");
            });
        }
    }
    ...
    let (pm_ep, bootstrap_authority_ep) =
        unsafe { boot_services(mgr, ut, total_usable, CAP_ROOTFS_READY_NTFN) };

    // Service monitor loop
    trona::uinfo!(|_lb| {
        _lb.str(b"[INIT] Entering service monitor loop\n");
    });
    unsafe { service_monitor(mgr, pm_ep, bootstrap_authority_ep, CAP_ROOTFS_READY_NTFN) };
}
