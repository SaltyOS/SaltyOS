...
use super::{CAP_SELF_CSPACE, CAP_SELF_TCB, CAP_SELF_VSPACE};
...
unsafe extern "C" fn thread2_entry() {
    unsafe {
        ...
        trona::uinfo!(|_lb| {
            _lb.str(b"[THREAD2] started, waiting on endpoint\n");
        });

        let mut msg = TronaMsg::zeroed();
        let mut badge: u64 = 0;

        let err = ipc::recv_ctx(
            &raw mut THREAD2_IPC_CTX,
            CAP_TEST_EP,
            &raw mut msg,
            &raw mut badge,
        );
        if err == 0 {
            trona::uinfo!(|_lb| {
                _lb.str(b"[THREAD2] received message! label=");
                _lb.hex(msg.label);
                _lb.str(b" reg0=");
                _lb.hex(msg.regs[0]);
                _lb.str(b"\n");
            ...
            trona::uerror!(|_lb| {
                _lb.str(b"[THREAD2] recv failed, error=");
                _lb.hex(err as u64);
                _lb.str(b"\n");
            });
        }
...
unsafe extern "C" fn fault_handler_entry() {
    unsafe {
        ipc::ipc_context_init(
            ...
            super::IPC_BUF_VADDR as *mut IpcBuffer,
        );
        trona::uinfo!(|_lb| {
            _lb.str(b"[FAULT_HANDLER] started, waiting for fault\n");
        });
        ...
        if err != 0 {
            trona::uerror!(|_lb| {
                _lb.str(b"[FAULT_HANDLER] recv failed err=");
                _lb.hex(err as u64);
                _lb.str(b"\n");
            });
        } else {
            trona::uinfo!(|_lb| {
                _lb.str(b"[FAULT_HANDLER] received fault! mapping page...\n");
            });
            ...
            if merr != 0 {
                trona::uerror!(|_lb| {
                    _lb.str(b"[FAULT_HANDLER] vspace_map failed err=");
                    _lb.hex(merr as u64);
                    _lb.str(b"\n");
                });
            } else {
                trona::uinfo!(|_lb| {
                    _lb.str(b"[FAULT_HANDLER] page mapped OK\n");
                });
            }
...
pub unsafe fn phase1_ipc_test(ut: Cap) -> i32 {
    trona::uinfo!(|_lb| {
        _lb.str(b"[INIT] Phase 1: IPC test\n");
    });
    ...
    unsafe {
        ...
        trona::uinfo!(|_lb| {
            _lb.str(b"[INIT] Endpoint created\n");
        });
        ...
        if err != 0 {
            trona::uerror!(|_lb| {
                _lb.str(b"[INIT] FAIL: TCB_SET_SPACE error=");
                _lb.hex(err as u64);
                _lb.str(b"\n");
            });
            return -1;
        ...
        if err != 0 {
            trona::uerror!(|_lb| {
                _lb.str(b"[INIT] FAIL: TCB_CONFIGURE error=");
                _lb.hex(err as u64);
                _lb.str(b"\n");
            });
            return -1;
        ...
        if err != 0 {
            trona::uerror!(|_lb| {
                _lb.str(b"[INIT] WARN: thread2 IPC buf frame retype err\n");
            });
        } else {
        ...
        if err != 0 {
            trona::uerror!(|_lb| {
                _lb.str(b"[INIT] FAIL: SC_CONFIGURE error=");
                _lb.hex(err as u64);
                _lb.str(b"\n");
            });
            return -1;
        ...
        if err != 0 {
            trona::uerror!(|_lb| {
                _lb.str(b"[INIT] FAIL: SC_BIND error=");
                _lb.hex(err as u64);
                _lb.str(b"\n");
            });
            return -1;
        ...
        if err != 0 {
            trona::uerror!(|_lb| {
                _lb.str(b"[INIT] FAIL: TCB_RESUME error=");
                _lb.hex(err as u64);
                _lb.str(b"\n");
            });
            return -1;
        ...
        trona::uinfo!(|_lb| {
            _lb.str(b"[INIT] Sending test message to endpoint\n");
        });
        let mut msg = TronaMsg::zeroed();
        ...
        if err != 0 {
            trona::uerror!(|_lb| {
                _lb.str(b"[INIT] FAIL: send error=");
                _lb.hex(err as u64);
                _lb.str(b"\n");
            });
            return -1;
        ...
        trona::uinfo!(|_lb| {
            _lb.str(b"[INIT] Message sent successfully!\n");
        });
        trona::uinfo!(|_lb| {
            _lb.str(b"[INIT] Phase 1 IPC test PASSED\n");
        });
...
pub unsafe fn phase2_fault_test(ut: Cap) -> i32 {
    ...
    unsafe {
        ...
        if err != 0 {
            trona::uerror!(|_lb| {
                _lb.str(b"[INIT] FAIL: fault TCB set_space err=");
                _lb.hex(err as u64);
                _lb.str(b"\n");
            });
            return -1;
        ...
        if err != 0 {
            trona::uerror!(|_lb| {
                _lb.str(b"[INIT] FAIL: fault TCB configure err=");
                _lb.hex(err as u64);
                _lb.str(b"\n");
            });
            return -1;
        ...
        trona::uinfo!(|_lb| {
            _lb.str(b"[INIT] Triggering page fault at ");
            _lb.hex(FAULT_TEST_ADDR);
            _lb.str(b"\n");
        });
        ...
        trona::uinfo!(|_lb| {
            _lb.str(b"[INIT] Resumed after fault! val=");
            _lb.hex(val);
            _lb.str(b"\n");
        });
        trona::uinfo!(|_lb| {
            _lb.str(b"[INIT] Phase 2 Fault test PASSED\n");
        });
        0
    }
}
