...
//! Per-client state management and object-slot cleanup on exit.
...
use crate::posix::socket::close_socket;
use crate::types::*;
use crate::{
    ...
    unsafe {
        ...
        for i in 0..max_clients() {
            if CLIENTS!()[i].active == 0 {
                ...
                CLIENTS!()[i].active = 1;
                // Allocate objects and POSIX extension state if not yet allocated.
                if CLIENTS!()[i].objects.is_null() {
                    let fds_ptr = vfs_alloc_array::<ObjectEntry>(INITIAL_FDS);
                    let ext_ptr = vfs_alloc_array::<PosixObjExt>(INITIAL_FDS);
                    if fds_ptr.is_null() || ext_ptr.is_null() {
                        CLIENTS!()[i].active = 0;
                        return core::ptr::null_mut();
                    }
                    CLIENTS!()[i].objects = fds_ptr;
                    CLIENTS!()[i].objects_cap = INITIAL_FDS as u16;
                    CLIENTS!()[i].posix_ext = ext_ptr;
                }
                for j in 0..CLIENTS!()[i].objects_cap as usize {
                    *CLIENTS!()[i].objects.add(j) = ObjectEntry::zeroed();
                    *CLIENTS!()[i].posix_ext.add(j) = PosixObjExt::zeroed();
                }
                // Initialize cwd to "/"
                CLIENTS!()[i].cwd[0] = b'/';
                let mut k = 1;
...
pub(crate) unsafe fn object_set_cloexec(entry: *mut ObjectEntry, enabled: bool) {
    unsafe {
        if enabled {
            (*entry).flags |= OBJ_FLAG_CLOEXEC;
        } else {
            (*entry).flags &= !OBJ_FLAG_CLOEXEC;
        }
    }
...
pub(crate) unsafe fn object_descriptor_flags(entry: *const ObjectEntry) -> u64 {
    unsafe {
        if ((*entry).flags & OBJ_FLAG_CLOEXEC) != 0 {
            1
        } else {
            0
        }
...
pub(crate) unsafe fn cleanup_client_state(dead_badge: u64) {
    unsafe {
        ...
        // Close every open object slot owned by the dead client to drop pipe/socket refs.
        for i in 0..(*cli).objects_cap as usize {
            let fde = (*cli).objects.add(i);
            let ext = (*cli).posix_ext.add(i);
            if (*fde).active == 0 {
                continue;
            }
            match (*fde).obj_type {
                OBJ_TYPE_SOCKET => close_socket(fde, ext),
                OBJ_TYPE_PIPE => close_pipe(fde, ext),
                OBJ_TYPE_EPOLL => {
                    let ep_idx = (*ext).epoll_id as usize;
            ...
            *(*cli).objects.add(i) = ObjectEntry::zeroed();
            *(*cli).posix_ext.add(i) = PosixObjExt::zeroed();
        }
        ...
        // Notify posix_ttysrv to release any controlling terminal owned by this dead client.
        let mut treq = TronaMsg::zeroed();
        treq.label = POSIX_TTYSRV_CLIENT_EXIT;
        treq.regs[0] = dead_badge;
        treq.length = 1;
        let _ = ipc::nbsend_ctx(ipc_ctx(), VFS_CAP_POSIX_TTYSRV_EP, &raw const treq);
