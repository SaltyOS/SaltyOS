...
use crate::posix::procfs::{handle_proc_open, handle_proc_read, handle_proc_readdir, handle_proc_stat};
use crate::ramfs::{
    ...
    unsafe {
        ...
        for fd in 0..(*cli).objects_cap as usize {
            if (*(*cli).objects.add(fd)).active == 0 {
                ...
                (*(*cli).objects.add(fd)).obj_type = OBJ_TYPE_MOUNT;
                (*(*cli).posix_ext.add(fd)).inode = 0;
                (*(*cli).objects.add(fd)).offset = 0;
                (*(*cli).posix_ext.add(fd)).dir_cursor = 0;
                (*(*cli).posix_ext.add(fd)).mount_idx = mount_idx as u8;
                (*(*cli).posix_ext.add(fd)).mount_remote_ino = remote_ino;
                (*(*cli).objects.add(fd)).dev_type = 0;
                (*(*cli).objects.add(fd)).flags = object_open_flags(flags);
                (*(*cli).posix_ext.add(fd)).mount_batch_count = 0;
                (*(*cli).posix_ext.add(fd)).mount_batch_index = 0;
                (*(*cli).posix_ext.add(fd)).mount_batch_next_cursor = 0;
        ...
        for fd in 0..(*cli).objects_cap as usize {
            if (*(*cli).objects.add(fd)).active == 0 {
                ...
                if (*inode).ftype == FTYPE_CHAR_DEVICE {
                    (*(*cli).objects.add(fd)).obj_type = OBJ_TYPE_DEVICE;
                    (*(*cli).objects.add(fd)).dev_type = (*inode).dev_type;
                    if (*inode).dev_type == DEV_PTY_SLAVE {
                        (*(*cli).posix_ext.add(fd)).pty_id = (*inode).size as u32;
                    }
                } else if (*inode).ftype == FTYPE_DIRECTORY {
                    (*(*cli).objects.add(fd)).obj_type = OBJ_TYPE_DIR;
                } else if (*inode).ftype == FTYPE_FIFO {
                    let pipe_id = (*inode).size as u32;
                    let pipe = find_pipe(pipe_id);
                    if pipe.is_null() {
                        (*(*cli).objects.add(fd)).active = 0;
                        (*reply).label = TRONA_INVALID_OPERATION;
                        return;
                    }
                    (*(*cli).objects.add(fd)).obj_type = OBJ_TYPE_PIPE;
                    (*(*cli).posix_ext.add(fd)).pipe_id = pipe_id;
                    if flags_allow_write(flags) {
                        (*(*cli).objects.add(fd)).flags = object_open_flags(O_WRONLY | (flags & O_NONBLOCK));
                        (*pipe).write_refcount += 1;
                    } else {
                        (*(*cli).objects.add(fd)).flags = object_open_flags(flags & O_NONBLOCK);
                        (*pipe).read_refcount += 1;
                    }
                } else {
                    (*(*cli).objects.add(fd)).obj_type = OBJ_TYPE_FILE;
                    if (flags & O_APPEND) != 0 {
                        (*(*cli).objects.add(fd)).offset = (*inode).size;
                    }
                }
            ...
            if (*(*cli).objects.add(fd)).active == 0 {
                ...
                if (*inode).ftype == FTYPE_CHAR_DEVICE {
                    ...
                    if flags_allow_write(flags) {
                        ...
                        (*(*cli).objects.add(fd)).flags = object_open_flags(flags & O_NONBLOCK); // O_RDONLY
                        (*pipe).read_refcount += 1;
                    }
                } else {
                    (*(*cli).objects.add(fd)).obj_type = OBJ_TYPE_FILE;
...
pub(crate) unsafe fn handle_read(msg: *const TronaMsg, reply: *mut TronaMsg, badge: u64) {
    unsafe {
        ...
        if !flags_allow_read((*(*cli).objects.add(fd as usize)).flags) {
            (*reply).label = TRONA_INVALID_OPERATION;
            return;
        }

        let fde = &mut *(*cli).objects.add(fd as usize);
        let ext = &mut *(*cli).posix_ext.add(fd as usize);

        match fde.obj_type {
            OBJ_TYPE_DEVICE => match fde.dev_type {
                DEV_CONSOLE => {
            ...
            OBJ_TYPE_FILE => {
                ...
                if (*inode).ftype == FTYPE_PROC_FILE {
                    ...
                    (*(*cli).objects.add(fd as usize)).offset += bytes_read;
                    return;
                }
...
pub(crate) unsafe fn handle_write(msg: *const TronaMsg, reply: *mut TronaMsg, badge: u64) {
    unsafe {
        ...
        if !flags_allow_write((*(*cli).objects.add(fd as usize)).flags) {
            (*reply).label = TRONA_INVALID_OPERATION;
            return;
        }
        ...
        match fde.obj_type {
            OBJ_TYPE_DEVICE => match fde.dev_type {
                ...
                DEV_PTY_SLAVE => {
                    // Forward write to posix_ttysrv for OPOST processing + serial/display output
                    let pty_id = ext.pty_id as u64;
                    let src = &(*msg).regs[2] as *const u64 as *const u8;
                    let mut sent: u64 = 0;
                    while sent < count {
            ...
            OBJ_TYPE_FILE => {
                ...
                crate::posix::misc::notify_mmsrv_mmap_write(
                    &*fde,
                    &*ext,
                    offset,
                    count,
...
/// Positioned read: read at an explicit offset without updating the slot cursor.
/// IPC: regs[0]=fd, regs[1]=count, regs[2]=offset (i64).
/// Supported for OBJ_TYPE_FILE and OBJ_TYPE_MOUNT.
pub(crate) unsafe fn handle_pread(msg: *const TronaMsg, reply: *mut TronaMsg, badge: u64) {
    unsafe {
        ...
        match fde.obj_type {
            ...
            OBJ_TYPE_MOUNT => {
                let mount_idx = ext.mount_idx as usize;
                let remote_ino = ext.mount_remote_ino;
        ...
        // Note: slot cursor (fde.offset) is NOT updated.
    }
}

/// Positioned write: write at an explicit offset without updating the slot cursor.
...
pub(crate) unsafe fn handle_close(msg: *const TronaMsg, reply: *mut TronaMsg, badge: u64) {
    unsafe {
        ...
        if fde.obj_type == OBJ_TYPE_SHM && fde.offset != 0 {
            let inode = inode_by_ino(ext.inode);
        ...
        if fde.obj_type != OBJ_TYPE_PIPE && fde.obj_type != OBJ_TYPE_SOCKET {
            inode_close(ext.inode);
        }

        (*(*cli).objects.add(fd as usize)).active = 0;
        *(*cli).posix_ext.add(fd as usize) = PosixObjExt::zeroed();
        (*reply).label = TRONA_OK;
    }
}
...
pub(crate) unsafe fn handle_lseek(msg: *const TronaMsg, reply: *mut TronaMsg, badge: u64) {
    unsafe {
        ...
        let fdt = (*(*cli).objects.add(fd as usize)).obj_type;
        if fdt != OBJ_TYPE_FILE && fdt != OBJ_TYPE_MOUNT {
            (*reply).label = TRONA_INVALID_OPERATION;
            return;
        }
        ...
        let file_size: u64 = if fdt == OBJ_TYPE_MOUNT {
            let fde = &*(*cli).objects.add(fd as usize);
            match mount_stat(ext.mount_idx as usize, ext.mount_remote_ino) {
                Some((size, _, _, _, _)) => size,
                None => 0,
            }
        } else {
            let inode = inode_by_ino((*(*cli).posix_ext.add(fd as usize)).inode);
            if inode.is_null() {
                (*reply).label = TRONA_INVALID_ARGUMENT;
                return;
            }
        ...
        let new_offset: i64 = match whence {
            0 => offset,                                                // SEEK_SET
            1 => (*(*cli).objects.add(fd as usize)).offset as i64 + offset, // SEEK_CUR
        ...
        (*(*cli).objects.add(fd as usize)).offset = new_offset as u64;
        (*reply).label = TRONA_OK;
        (*reply).length = 1;
...
pub(crate) unsafe fn handle_fstat(msg: *const TronaMsg, reply: *mut TronaMsg, badge: u64) {
    unsafe {
        ...
        if (*(*cli).objects.add(fd as usize)).obj_type == OBJ_TYPE_MOUNT {
            let fde = &*(*cli).objects.add(fd as usize);
            let mount_idx = ext.mount_idx as usize;
            let remote_ino = ext.mount_remote_ino;
                ...
                if m.active != 0 && m.mount_ino == (*inode).ino {
                    ...
                    for fd in 0..(*cli).objects_cap as usize {
                        if (*(*cli).objects.add(fd)).active == 0 {
                            ...
                            (*(*cli).objects.add(fd)).offset = 0;
                            (*(*cli).posix_ext.add(fd)).dir_cursor = 0;
                            (*(*cli).posix_ext.add(fd)).mount_idx = i as u8;
                            (*(*cli).posix_ext.add(fd)).mount_remote_ino = m.root_ino as u64;
                            (*(*cli).objects.add(fd)).dev_type = 0;
                            (*(*cli).objects.add(fd)).flags = 0;
                            (*(*cli).posix_ext.add(fd)).mount_batch_count = 0;
                            (*(*cli).posix_ext.add(fd)).mount_batch_index = 0;
                            (*(*cli).posix_ext.add(fd)).mount_batch_next_cursor = 0;
                            (*reply).label = TRONA_OK;
...
pub(crate) unsafe fn handle_readdir(msg: *const TronaMsg, reply: *mut TronaMsg, badge: u64) {
    unsafe {
        ...
        if cli.is_null()
            ...
            || fd >= (*cli).objects_cap as i32
            || (*(*cli).objects.add(fd as usize)).active == 0
            || (*(*cli).objects.add(fd as usize)).obj_type != OBJ_TYPE_DIR
        {
            (*reply).label = TRONA_INVALID_ARGUMENT;
            return;
        }

        let dir = inode_by_ino((*(*cli).posix_ext.add(fd as usize)).inode);
        if dir.is_null() {
            (*reply).label = TRONA_INVALID_ARGUMENT;
            return;
        }
        ...
        if (*dir).ftype == FTYPE_PROC_FILE {
            let cursor = (*(*cli).posix_ext.add(fd as usize)).dir_cursor;
            ...
            if (*reply).label == TRONA_OK && (*reply).regs[0] != 0 {
                ...
                (*(*cli).posix_ext.add(fd as usize)).dir_cursor = (*reply).regs[1] as u32;
        ...
        if fde.offset == 0 {
            ...
            if (*dir).ino == crate::consts::ROOT_INO && ul_idx >= 0 {
                fde.offset = 1;
                ext.dir_cursor = 0;
                ext.mount_batch_count = 0;
                ext.mount_batch_index = 0;
                ext.mount_batch_next_cursor = 0;
        ...
        if fde.offset == 1 {
            ...
            if ul_idx >= 0 {
                ...
                crate::mount::mount_readdir(mi, ext, root_remote_ino, reply);
                if (*reply).regs[0] != 0 {
                    return;
                }
            }
