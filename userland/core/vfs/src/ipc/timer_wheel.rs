...
/// Pop and dispatch all expired timers. Called by the timer owner after
...
pub(crate) fn process_expired_timers() {
    ...
    for i in 0..count {
        match expired[i].kind {
            TimerKind::PollDeadline => unsafe {
                poll::handle_poll_timer(expired[i].cookie, now_ns);
            },
            TimerKind::PtyReadTimeout => unsafe {
                crate::personality::posix::tty::handle_pty_timer(expired[i].cookie, now_ns);
            },
    ...
    unsafe {
        if did_mount {
            if !crate::owner::loop_::OWNER_STATE_PTR.is_null() {
                crate::boot::late_mount::retry_pending_mounts(
                    &mut *crate::owner::loop_::OWNER_STATE_PTR,
                );
            }
        }
    }
...
pub(crate) fn try_claim_timer(worker_idx: u32) -> bool {
    ...
    if TIMER_OWNER
        .compare_exchange(current_owner, worker_idx, Ordering::AcqRel, Ordering::Relaxed)
        .is_ok()
    {
