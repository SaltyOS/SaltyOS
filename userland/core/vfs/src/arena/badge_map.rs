...
//
//! `BadgeMap` — O(1) badge-to-client lookup via open-addressing hash
//! table. Stores `(badge, slot, epoch)` triples so the consumer can
//! reconstruct a `Handle<ClientState>` without BadgeMap knowing
//! about ClientState.
//!
//! Capacity grows on demand once the live-entry load factor crosses
//! 75%. There is no fixed cap — each grow doubles the bucket array
//! and rehashes. Backing memory is anonymous mmsrv-mapped pages.
...
/// Open-addressing hash map: `badge -> (slot, epoch)`. Capacity is
/// always a power of two; the table grows on demand when the
...
impl BadgeMap {
    ...
    fn alloc_entries(cap: u32) -> Option<*mut Entry> {
        ...
        if ptr.is_null() || ptr == usize::MAX as *mut u8 {
            ...
            let entries = ptr as *mut Entry;
            for i in 0..cap {
                unsafe {
                    *entries.add(i as usize) = Entry::EMPTY;
                }
            }
            Some(entries)
        }
    ...
    /// Create a new BadgeMap with the given minimum capacity (rounded
    /// up to power of 2, and floored at 16). Returns `None` if the
    ...
    /// Insert or update a mapping. Triggers a `grow` first if the
    /// load factor would cross 75% with this insertion.
    pub(crate) fn insert(&mut self, badge: u64, slot: u32, epoch: u32) -> Result<(), NoSpace> {
        if badge == EMPTY_BADGE || badge == TOMBSTONE_BADGE {
            return Err(NoSpace);
        }
        if self.lookup(badge).is_none()
            && self.len().saturating_add(1).saturating_mul(4) >= self.capacity().saturating_mul(3)
        {
            self.grow()?;
        }
    ...
    /// Remove a badge mapping. Returns `true` if it was found.
    ...
    /// take the upper bits. Distributes sequential and semi-random
    /// badge values evenly.
    #[inline]
