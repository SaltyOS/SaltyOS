...
//! is valid only when its epoch matches the slot's current epoch in
//! the arena. Stale handles are cheaply rejected without touching the
//! data itself.
...
impl<T> Handle<T> {
    /// Sentinel value representing "no object". Slot `u32::MAX` is
    /// never allocated by an arena.
