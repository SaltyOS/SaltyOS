...
use crate::vfs_core::mount_ctl::{self, set_mount_identity};
use crate::vfs_core::error::{VfsError, VfsResult};
use crate::vfs_core::mount::{Mount, MountHandle, MNT_BIND, MNT_RBIND};
use crate::vfs_core::vnode::{VnodeHandle, VN_COVERED, VN_ROOT, VT_DIR};
...
fn is_boot_scaffold_child_mount(mount_path: &[u8]) -> bool {
    for path in BOOT_SCAFFOLD_CHILD_MOUNTS {
        if mount_path == path {
            return true;
        }
    }
...
unsafe fn lookup_mountpoint_under_root(
    ...
    while pos < mount_path.len() {
        ...
        unsafe {
            let ctx = mount_ctl::build_vop_context(state, current)
                .ok_or(VfsError::Io)?;
            let ops = &*(*ctx.vnode).ops;
            let next = (ops.meta.lookup)(&ctx, comp.as_ptr(), comp.len() as u8)?;
...
pub(crate) unsafe fn vfs_pivot_root(
    ...
    for i in 0..scan_count {
        ...
        let mount_path_len = {
            ...
            if mp.parent.slot() != old_root_mh.slot() {
                continue;
            }
        ...
        if !is_boot_scaffold_child_mount(mount_path) {
            continue;
        }
        ...
        let target_vh = match unsafe {
            lookup_mountpoint_under_root(state, new_root_vh, mount_path)
        } {
            Ok(vh) => vh,
            Err(e) => {
                trona::uerror!(|_lb| {
    ...
    state.root_mount = new_root_mh;
    {
        ...
        new_mp.covered_vnode = VnodeHandle::INVALID;
        new_mp.parent = MountHandle::INVALID;
    }
    ...
    for i in 0..child_count {
        ...
        // Re-parent and wire new covering link.
        {
            ...
            mp.parent = new_root_mh;
            mp.covered_vnode = new_target;
        }
    ...
    // ---- Step 4: Attach old root at put_old ----
    {
        ...
        old_mp.covered_vnode = put_old_vh;
        old_mp.parent = new_root_mh;
...
pub(crate) unsafe fn vfs_bind_mount(
    ...
    // Get source mount info.
    let source_vn = state.vnodes.get(source_vh).ok_or(VfsError::Inval)?;
    let source_mh = source_vn.mount;
    if !source_mh.is_valid() {
        return Err(VfsError::Inval);
    }
...
unsafe fn rbind_clone_children(
    ...
    for i in 0..count {
        ...
        let child_covered = child_mp.covered_vnode;
        ...
        cloned.parent = new_parent_mh;
        cloned.root_vnode = child_root_vh;
        cloned.covered_vnode = child_covered;
