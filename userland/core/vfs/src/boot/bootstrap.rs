...
use trona::types::core::CpioEntryExt;
...
unsafe fn boot_mkdir(
    ...
    unsafe {
        let ctx = mount_ctl::build_vop_context(state, parent_vh).ok_or(VfsError::Io)?;
...
unsafe fn boot_write(
    ...
    unsafe {
        ...
        ((*ops).data.write)(&data_ctx, offset, data.as_ptr(), data.len() as u64)
    }
}
...
pub(crate) unsafe fn boot_lookup(
    ...
    unsafe {
        let ctx = mount_ctl::build_vop_context(state, parent_vh).ok_or(VfsError::Io)?;
        let ops = &*(*ctx.vnode).ops;
        let result = (ops.meta.lookup)(&ctx, name.as_ptr(), name.len() as u8);
        mount_ctl::clear_trampolines();
        result
    }
}
...
unsafe fn stage3_mount_root_ramfs(state: &mut VfsState) -> VfsResult<VnodeHandle> {
    unsafe {
        let mh = mount_ctl::do_mount_with_ops(
            state,
            VnodeHandle::INVALID,
    ...
    if saltyfs_mounted {
        ...
        unsafe {
            ...
            if saltyfs_mh.is_valid() {
                ...
                let put_old_vh =
                    match boot_ensure_dir(state, saltyfs_root_vh, b"initramfs", S_IFDIR_L | 0o555)
                    {
                        Ok(vh) => vh,
                        Err(_) => {
                            super::late_mount::note_root_failed();
