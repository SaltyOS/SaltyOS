...
pub(crate) struct SysctlfsVnodeData {
    ...
    _pad: [u8; 7],
    /// For Node: pointer to the `SysctlNode` in the MIB tree.
    /// For Leaf: pointer to the `SysctlLeaf` (cast via *const SysctlNode).
...
/// Encode a sysctlfs vnode id from kind and a pointer hash.
///
...
#[inline]
pub(super) fn encode_id(kind: SysctlfsKind, ptr: *const SysctlNode) -> u64 {
