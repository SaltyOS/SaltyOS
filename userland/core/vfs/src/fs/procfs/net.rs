...
//
//! Network-related `/proc` file generators
//! (`/proc/net/*`, `/etc/hosts`, `/etc/resolv.conf` content).
//!
//! All routines query netsrv through `NAMESRV_LOOKUP`-resolved
//! `caps::netsrv_ep()` and consume the typed reply layout
//! documented at the `NET_GET_CONFIG` / `NET_GET_ARP_ENTRY`
//! label sites.

use trona_kernel::core_types::{IpcContext, TronaMsg};
use trona_kernel::ipc::mp_call_ctx;
use trona_protocol::common::TRONA_OK;
use trona_protocol::netsrv::{NET_GET_ARP_ENTRY, NET_GET_CONFIG};
...
unsafe fn netsrv_get_config(info: &mut NetConfigInfo) -> bool {
    unsafe {
        ...
        let err = mp_call_ctx(
            ipc_ctx(),
            trona_runtime::client::caps::netsrv_ep().addr(),
            &raw const msg,
            &raw mut reply,
