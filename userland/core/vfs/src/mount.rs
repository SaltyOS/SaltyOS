...
pub(crate) unsafe fn mount_readdir_emit_cached(ext: *mut PosixObjExt, reply: *mut TronaMsg) {
    unsafe {
        let e = &mut *ext;
        let idx = e.mount_batch_index as usize;
        let ent = &e.mount_batch[idx];
        ...
        e.mount_batch_index = e.mount_batch_index.saturating_add(1);
        if e.mount_batch_index >= e.mount_batch_count {
            e.mount_batch_index = 0;
            e.mount_batch_count = 0;
            e.dir_cursor = if e.mount_batch_next_cursor == 0 {
                u32::MAX
            } else {
                e.mount_batch_next_cursor
            };
        }
    }
}
...
pub(crate) unsafe fn mount_readdir(
    ...
    unsafe {
        ...
        if e.mount_batch_count > 0 && e.mount_batch_index < e.mount_batch_count {
            mount_readdir_emit_cached(ext, reply);
            return;
        }
        ...
        req.regs[0] = dir_ino;
        req.regs[1] = e.dir_cursor as u64;
        req.length = 2;
        ...
        for n in 0..take {
            ...
            e.mount_batch[n].ino = child_ino;
            e.mount_batch[n].d_type = d_type;
            e.mount_batch[n].name_len = name_len;
            e.mount_batch[n].name = name;
        }

        e.mount_batch_count = take as u8;
        e.mount_batch_index = 0;
        e.mount_batch_next_cursor = next_cursor;
        mount_readdir_emit_cached(ext, reply);
    }
}
...
pub(crate) unsafe fn setup_saltyfs_mount() {
    unsafe {
        ...
        if err != 0 || ns_reply.label != TRONA_OK {
            trona::udebug!(|_lb| {
                _lb.str(b"[VFS] saltyfs not found in namesrv (ok if no data disk)\n");
            });
            return;
        }

        trona::uinfo!(|_lb| {
            _lb.str(b"[VFS] Found saltyfs endpoint via namesrv\n");
        });
