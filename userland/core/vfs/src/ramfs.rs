...
pub(crate) unsafe fn grow_writable_pool() -> i32 {
    unsafe {
        ...
        let new_used_ptr = crate::vfs_mem::map_anon((used_pages * 4096) as u64);
        ...
        if !old_used_ptr.is_null() {
            ...
            crate::vfs_mem::unmap(old_used_ptr, (old_used_pages * 4096) as u64);
        }
        ...
        let new_next_ptr = crate::vfs_mem::map_anon((next_pages * 4096) as u64);
        ...
        if !old_next_ptr.is_null() {
            ...
            crate::vfs_mem::unmap(old_next_ptr as *mut u8, (old_next_pages * 4096) as u64);
        }
