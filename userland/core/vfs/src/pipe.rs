...
//! Pipe subsystem and object-slot duplication (dup/dup2/dup3).
...
use crate::client::{
    get_client, get_client_noalloc, object_open_flags, object_set_cloexec,
    object_status_flags,
};
use crate::consts::*;
use crate::ramfs::{inode_close, inode_open};
use crate::posix::socket::{alloc_reply_slot, close_socket, find_socket};
use crate::types::*;
use crate::{
...
pub(crate) unsafe fn close_pipe(fde: *mut ObjectEntry, ext: *mut PosixObjExt) {
    unsafe {
        let pipe = find_pipe((*ext).pipe_id());
        if pipe.is_null() {
            return;
        }
        ...
        if is_read_end {
            ...
            if (*pipe).read_refcount == 0 {
                while let Some(w) = pipe_pop_write_waiter(pipe) {
                    ...
                    wake.label = TRONA_INVALID_OPERATION; // EPIPE
...
/// `is_read_end`: true = wake waiters on read-end objects, false = wake waiters on write-end objects.
    ...
    unsafe {
        ...
        for i in 0..max_poll_waiters() {
            ...
            for j in 0..POLL_WAITERS!()[i].nfds as usize {
                let pfd = POLL_WAITERS!()[i].objects[j].0;
                if pfd < 0 || pfd >= (*cli).objects_cap as i32 {
                    continue;
                }
                let fde = *(*cli).objects.add(pfd as usize);
                let ext = *(*cli).posix_ext.add(pfd as usize);
                if fde.active == 0 || fde.obj_type != OBJ_TYPE_PIPE {
                    continue;
                }
                if ext.pipe_id() != pipe_id {
                    continue;
                }
                ...
                let requested = POLL_WAITERS!()[i].objects[j].1;
...
pub(crate) unsafe fn handle_pipe(msg: *const TronaMsg, reply: *mut TronaMsg, badge: u64) {
    unsafe {
        ...
        for i in 0..(*cli).objects_cap as usize {
        ...
        for i in 0..(*cli).objects_cap as usize {
            if (*(*cli).objects.add(i)).active == 0 && i as i32 != read_fd {
                write_fd = i as i32;
                break;
            }
        }
        ...
        // Set up read-end fd
        (*(*cli).objects.add(read_fd as usize)).active = 1;
        (*(*cli).objects.add(read_fd as usize)).obj_type = OBJ_TYPE_PIPE;
        (*(*cli).posix_ext.add(read_fd as usize)).pipe_id = (*pipe).pipe_id;
        (*(*cli).objects.add(read_fd as usize)).flags = object_open_flags(
            if flags & O_NONBLOCK != 0 { O_NONBLOCK } else { 0 }
                | (flags & O_CLOEXEC),
        );
        (*(*cli).objects.add(read_fd as usize)).offset = 0;

        // Set up write-end fd
        (*(*cli).objects.add(write_fd as usize)).active = 1;
        (*(*cli).objects.add(write_fd as usize)).obj_type = OBJ_TYPE_PIPE;
        (*(*cli).posix_ext.add(write_fd as usize)).pipe_id = (*pipe).pipe_id;
        (*(*cli).objects.add(write_fd as usize)).flags = object_open_flags(
            O_WRONLY
        ...
        (*(*cli).objects.add(write_fd as usize)).offset = 0;
...
pub(crate) unsafe fn handle_dup(msg: *const TronaMsg, reply: *mut TronaMsg, badge: u64) {
    unsafe {
        let oldfd = (*msg).regs[0] as i32;
        let cli = get_client(badge);
        if cli.is_null()
            || oldfd < 0
            || oldfd >= (*cli).objects_cap as i32
            || (*(*cli).objects.add(oldfd as usize)).active == 0
        {
            (*reply).label = TRONA_INVALID_ARGUMENT;
            return;
        }

        // Find the lowest free object slot.
        let mut newfd: i32 = -1;
        for i in 0..(*cli).objects_cap as usize {
            if (*(*cli).objects.add(i)).active == 0 {
                newfd = i as i32;
        ...
        dup_object_entry(cli, oldfd, newfd);
...
pub(crate) unsafe fn handle_dup2(msg: *const TronaMsg, reply: *mut TronaMsg, badge: u64) {
    unsafe {
        let oldfd = (*msg).regs[0] as i32;
        ...
        // Close newfd if open
        if (*(*cli).objects.add(newfd as usize)).active != 0 {
            let fde = (*cli).objects.add(newfd as usize);
            let ext = (*cli).posix_ext.add(newfd as usize);
            if (*fde).obj_type == OBJ_TYPE_SOCKET {
                close_socket(fde, ext);
            } else if (*fde).obj_type == OBJ_TYPE_PIPE {
                close_pipe(fde, ext);
            } else {
                inode_close((*ext).inode);
            }
            (*(*cli).objects.add(newfd as usize)).active = 0;
            *ext = PosixObjExt::zeroed();
        }
...
pub(crate) unsafe fn handle_dup3(msg: *const TronaMsg, reply: *mut TronaMsg, badge: u64) {
    unsafe {
        let oldfd = (*msg).regs[0] as i32;
        let newfd = (*msg).regs[1] as i32;
        ...
        object_set_cloexec(
            (*cli).objects.add(newfd as usize),
            (flags & O_CLOEXEC) != 0,
        );
...
/// Copy an object slot entry and increment refcounts as needed.
pub(crate) unsafe fn dup_object_entry(cli: *mut ClientState, oldfd: i32, newfd: i32) {
    unsafe {
        *(*cli).objects.add(newfd as usize) = *(*cli).objects.add(oldfd as usize);
        *(*cli).posix_ext.add(newfd as usize) = *(*cli).posix_ext.add(oldfd as usize);
        (*(*cli).objects.add(newfd as usize)).flags =
            object_status_flags((*(*cli).objects.add(newfd as usize)).flags);
        let fde = *(*cli).objects.add(newfd as usize);
        let ext = *(*cli).posix_ext.add(newfd as usize);
        if fde.obj_type == OBJ_TYPE_PIPE {
            let pipe = find_pipe(ext.pipe_id());
            if !pipe.is_null() {
        ...
        } else if fde.obj_type == OBJ_TYPE_SOCKET {
            let sock = find_socket(ext.sock_id);
            if !sock.is_null() {
                (*sock).refcount += 1;
            }
        } else {
            inode_open(ext.inode);
        }
    }
}

pub(crate) unsafe fn handle_clone_fds(msg: *const TronaMsg, reply: *mut TronaMsg) {
    unsafe {
        ...
        // If parent has more object slots than child, grow the child's table to match.
        if (*parent).objects_cap > (*child).objects_cap {
            let required = (*parent).objects_cap as usize;
            let (new_fds, new_cap) =
                vfs_grow_array_with_min((*child).objects, (*child).objects_cap as usize, required);
            let (new_ext, _) =
                vfs_grow_array_with_min((*child).posix_ext, (*child).objects_cap as usize, required);
            if new_fds.is_null() || new_ext.is_null() {
                (*reply).label = TRONA_OUT_OF_MEMORY;
                return;
            }
            (*child).objects = new_fds;
            (*child).posix_ext = new_ext;
            (*child).objects_cap = new_cap as u16;
        }

        // Copy all object slots from parent to child.
        for i in 0..(*parent).objects_cap as usize {
            *(*child).objects.add(i) = *(*parent).objects.add(i);
            *(*child).posix_ext.add(i) = *(*parent).posix_ext.add(i);
            if (*(*child).objects.add(i)).active == 0 {
                continue;
            }
            // Increment pipe refcounts
            if (*(*child).objects.add(i)).obj_type == OBJ_TYPE_PIPE {
                let pipe = find_pipe((*(*child).posix_ext.add(i)).pipe_id());
                if !pipe.is_null() {
                    let is_read = ((*(*child).objects.add(i)).flags & O_ACCMODE) == 0;
                    if is_read {
                        (*pipe).read_refcount += 1;
                    } else {
                        (*pipe).write_refcount += 1;
            ...
            if (*(*child).objects.add(i)).obj_type == OBJ_TYPE_SOCKET {
                let sock = find_socket((*(*child).posix_ext.add(i)).sock_id);
                if !sock.is_null() {
                    (*sock).refcount += 1;
                }
            }
            ...
            if (*(*child).objects.add(i)).obj_type != OBJ_TYPE_PIPE
                && (*(*child).objects.add(i)).obj_type != OBJ_TYPE_SOCKET
            {
                inode_open((*(*child).posix_ext.add(i)).inode);
            }
        }
