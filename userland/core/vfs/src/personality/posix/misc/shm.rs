...
use crate::server::client::{extract_path, flags_append_writes, flags_nonblocking, object_open_flags};
...
unsafe fn shm_has_live_slots(
    ...
    state.clients.for_each_active(|client_handle, client| {
        for fd in 0..MAX_CLIENT_OBJECTS {
            ...
            let slot = &client.objects[fd];
            if !slot.is_live() || slot.kind() != ObjectKind::Shm {
                continue;
            }
            if unsafe { slot_shm_handle(state, slot) } == handle {
                found = true;
                return false;
            }
        }
...
pub(crate) unsafe fn ensure_shm_backing_pages(
    ...
    unsafe {
        ...
        let mut mm_msg = TronaMsg::zeroed();
        let mut mm_reply = TronaMsg::zeroed();
        mm_msg.label = if current_pages == 0 { MM_SHM_CREATE } else { MM_SHM_RESIZE };
        mm_msg.length = 2;
        mm_msg.regs[0] = handle.slot() as u64;
...
pub(crate) unsafe fn handle_shm_open(
    ...
    unsafe {
        ...
        if ni.vp.is_valid() {
            ...
            slot.rights = shm_rights_from_flags(flags);
            slot.flags = object_open_flags(flags);
            slot.append_on_write = if flags_append_writes(flags) { 1 } else { 0 };
            slot.nonblocking = if flags_nonblocking(flags) { 1 } else { 0 };
            slot.offset = 0;
            slot.set_shm(ni.vp, shm_id);
        ...
        slot.nonblocking = if flags_nonblocking(flags) { 1 } else { 0 };
        slot.offset = 0;
        slot.set_shm(new_vh, shm_handle.slot());
...
pub(crate) unsafe fn handle_shm_unlink(
    ...
    unsafe {
        ...
        let ops = &*(*ctx.vnode).ops;
        match (ops.meta.unlink)(&ctx, ni.last_name, ni.last_name_len) {
            Ok(()) => {
                mount_ctl::clear_trampolines();
