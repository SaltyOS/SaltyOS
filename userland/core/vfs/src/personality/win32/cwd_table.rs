...
impl Win32CwdTable {
    ...
    /// Ensure a Win32 client has a usable sidecar entry. A fresh
    /// entry starts on `C:` and seeds both `C:` and `Z:` to the
    /// namespace root; that mirrors the default drive projection
    /// used by the current Win32 subsystem while keeping room for
    /// per-drive cwd updates later.
    pub(crate) fn ensure_seeded(
        ...
        let blank = slot.drive_cwd.iter().all(|vkey| !vkey.is_valid());
        if blank {
            slot.current_drive = 2;
        }
        if root_vkey.is_valid() {
            if !slot.drive_cwd[2].is_valid() {
                slot.drive_cwd[2] = root_vkey;
            }
            if !slot.drive_cwd[25].is_valid() {
                slot.drive_cwd[25] = root_vkey;
            }
        }
        Ok(())
    }

    /// Copy Win32 cwd state across fork/clone. The destination is
    /// first seeded so a missing source still gets a valid default.
    pub(crate) fn clone_from(
        ...
        self.ensure_seeded(dst, root_vkey)?;
        let Some(src_slot) = self.get(src).copied() else {
            return Ok(());
        };
    ...
    /// Return the cwd anchor for a drive letter, if one has been
    /// established. Drive-relative NT paths (`C:foo`) use this
    /// anchor; drive-root paths (`C:\foo`) intentionally do not.
    pub(crate) fn drive_cwd_vkey(
        &self,
        client: ClientHandle,
        drive_index: usize,
    ) -> Option<VnodeKey> {
        ...
        let vkey = slot.drive_cwd[drive_index];
        if vkey.is_valid() { Some(vkey) } else { None }
    }
}
