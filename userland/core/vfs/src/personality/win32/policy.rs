...
fn win32_object_flags(desired_access: u32) -> u32 {
    ...
    let posix_like = match (can_read, can_write) {
        (false, true) => trona::consts::posix::O_WRONLY,
        (true, true) => trona::consts::posix::O_RDWR,
        _ => trona::consts::posix::O_RDONLY,
    };
...
pub(crate) fn win32_open_request(
    ...
    Some(OpenRequest {
        backend_open_flags: object_flags | if truncate_existing { trona::consts::posix::O_TRUNC } else { 0 },
        object_flags,
