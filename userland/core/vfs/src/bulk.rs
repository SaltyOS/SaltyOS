...
pub(crate) unsafe fn handle_bulk_read(
    ...
    unsafe {
        ...
        if fd < 0 || fd >= (*cli).objects_cap as i32 {
            (*reply).label = TRONA_INVALID_ARGUMENT;
            (*reply).length = 1;
            (*reply).regs[0] = 0;
            return;
        ...
        match fde.obj_type {
            OBJ_TYPE_MOUNT if *(&raw const crate::VFS_SHM_ACTIVE) => {
                bulk_read_mount(fde, ext, client_shm, shm_offset, capped, reply);
            }
            _ => {
                (*reply).label = TRONA_INVALID_ARGUMENT;
                (*reply).length = 1;
...
pub(crate) unsafe fn handle_bulk_pwrite(
    ...
    unsafe {
        ...
        match fde.obj_type {
            OBJ_TYPE_MOUNT if *(&raw const crate::VFS_SHM_ACTIVE) => {
                bulk_pwrite_mount(fde, ext, client_shm, shm_offset, capped, file_offset, reply);
            }
            _ => {
