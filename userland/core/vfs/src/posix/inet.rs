...
unsafe fn fd_is_nonblocking(cli: *mut ClientState, fd: i32) -> bool {
    unsafe { ((*(*cli).objects.add(fd as usize)).flags & O_NONBLOCK) != 0 }
}
...
unsafe fn find_inet_fd_by_conn(badge: u64, conn_id: u32) -> Option<i32> {
    unsafe {
        ...
        for fd in 0..(*cli).objects_cap as usize {
            let fde = &*(*cli).objects.add(fd);
            let ext = &*(*cli).posix_ext.add(fd);
            if fde.active != 0 && fde.obj_type == OBJ_TYPE_INET_SOCKET && ext.sock_id == conn_id {
...
/// Forward NET_SOCKET to netsrv, create OBJ_TYPE_INET_SOCKET fd.
    ...
    unsafe {
        ...
        for fd in 0..(*cli).objects_cap as usize {
            if (*(*cli).objects.add(fd)).active == 0 {
                ...
                (*(*cli).objects.add(fd)).obj_type = OBJ_TYPE_INET_SOCKET;
                (*(*cli).posix_ext.add(fd)).sock_id = conn_id;
...
pub(crate) unsafe fn handle_inet_connect(
    ...
    unsafe {
        ...
        let conn_id = (*(*cli).posix_ext.add(fd as usize)).sock_id;
...
pub(crate) unsafe fn close_inet_socket(fde: *mut ObjectEntry, ext: *mut PosixObjExt) {
    // SAFETY: Single-threaded VFS; IPC context valid.
    unsafe {
        let conn_id = (*ext).sock_id;
        let mut req = TronaMsg::zeroed();
