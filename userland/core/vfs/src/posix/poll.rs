...
use super::socket::{alloc_reply_slot, find_socket, sock_buf_free, sock_buf_len};
use crate::types::*;
use crate::{
        ...
        match waiter.kind {
            POLL_WAITER_KIND_POLL => {
                ...
                for i in 0..waiter.nfds as usize {
                    let rev = check_fd_readiness(cli, waiter.objects[i].0, waiter.objects[i].1 as u32) as u16;
            ...
            POLL_WAITER_KIND_EPOLL => {
                ...
                for i in 0..waiter.nfds as usize {
                    let rev = check_fd_readiness(cli, waiter.objects[i].0, waiter.objects[i].1 as u32);
                    if rev == 0 {
                        continue;
                    }
...
pub(crate) unsafe fn wake_poll_waiters(badge: u64, fd: i32, revents: u16) {
    unsafe {
        ...
        for i in 0..max_poll_waiters() {
            ...
            if fd == -1 {
                // Broadcast: report revents on all objects that requested matching events.
                ...
                let mut ready_count: u64 = 0;
                for j in 0..POLL_WAITERS!()[i].nfds as usize {
                    if POLL_WAITERS!()[i].objects[j].0 == fd {
...
pub(crate) unsafe fn check_fd_readiness(cli: *const ClientState, fd: i32, events: u32) -> u32 {
    unsafe {
        if fd < 0 || fd >= (*cli).objects_cap as i32 || (*(*cli).objects.add(fd as usize)).active == 0 {
        ...
        match fde.obj_type {
            ...
            OBJ_TYPE_DEVICE => {
                if fde.dev_type == DEV_PTY_SLAVE {
                    // Query posix_ttysrv for PTY readiness
                    let mut treq = TronaMsg::zeroed();
                    let mut treply = TronaMsg::zeroed();
                    treq.label = POSIX_TTYSRV_PTY_POLL;
            ...
            OBJ_TYPE_INET_SOCKET => {
                ...
                nreq.regs[0] = ext.sock_id as u64;
...
pub(crate) unsafe fn handle_epoll_create(reply: *mut TronaMsg, badge: u64) {
    unsafe {
        ...
        // Init object-slot entry.
        (*(*cli).objects.add(fd as usize)).active = 1;
        (*(*cli).objects.add(fd as usize)).obj_type = OBJ_TYPE_EPOLL;
        (*(*cli).posix_ext.add(fd as usize)).epoll_id = epoll_idx as u32;
        (*(*cli).posix_ext.add(fd as usize)).inode = 0;
        (*(*cli).objects.add(fd as usize)).offset = 0;
        (*(*cli).objects.add(fd as usize)).flags = 0;
...
pub(crate) unsafe fn handle_epoll_ctl(msg: *const TronaMsg, reply: *mut TronaMsg, badge: u64) {
    unsafe {
        ...
        if cli.is_null()
            || epfd < 0
            || epfd >= (*cli).objects_cap as i32
            || (*(*cli).objects.add(epfd as usize)).active == 0
            || (*(*cli).objects.add(epfd as usize)).obj_type != OBJ_TYPE_EPOLL
        {
            (*reply).label = TRONA_INVALID_ARGUMENT;
            return;
        }

        let ep_idx = (*(*cli).posix_ext.add(epfd as usize)).epoll_id as usize;
        ...
        for w in 0..max_poll_waiters() {
            if POLL_WAITERS!()[w].active == 0 {
                ...
                for i in 0..(*ep).entries_cap as usize {
                    ...
                    POLL_WAITERS!()[w].objects[n as usize].0 = (*ep.entries.add(i)).fd;
                    POLL_WAITERS!()[w].objects[n as usize].1 = (*ep.entries.add(i)).events as u16;
...
pub(crate) unsafe fn handle_poll(msg: *const TronaMsg, reply: *mut TronaMsg, badge: u64) -> bool {
    unsafe {
        ...
        for i in 0..actual_nfds as usize {
            ...
            if pfd < 0
                || pfd >= (*cli).objects_cap as i32
                || (*(*cli).objects.add(pfd as usize)).active == 0
            {
        ...
        for i in 0..max_poll_waiters() {
            if POLL_WAITERS!()[i].active == 0 {
                ...
                for j in 0..actual_nfds as usize {
                    POLL_WAITERS!()[i].objects[j].0 = (*msg).regs[2 + j * 2] as i32;
                    POLL_WAITERS!()[i].objects[j].1 = (*msg).regs[2 + j * 2 + 1] as u16;
