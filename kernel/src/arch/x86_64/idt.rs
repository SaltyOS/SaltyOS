        ...
        unsafe {
            ...
            if !current.is_null() && !(*current).fault_handler.is_null() {
...

                {
                    ...
                    // For #PF (vector 14), walk the page table and dump each level.
                    // This diagnoses missing mappings: the first zero entry shows
                    // which page table level is absent for the faulting address.
                    if f.vector == 14 {
                        use super::paging::PageTable;
                        let cr3 = super::paging::read_cr3();
                        let addr_mask: u64 = 0x000F_FFFF_FFFF_F000;
                        // Print CR3 to confirm which VSpace we're diagnosing.
                        // This distinguishes procmgr's VSpace from any child's.
                        s.puts("  VSpace CR3=");
                        s.hex(cr3 & addr_mask);
                        s.puts("\n");
                        // SAFETY: We are inside the outer `unsafe` block (line ~274).
                        // phys_to_virt returns a valid kernel virtual address for any
                        // physical address within the direct map. The CR3 and each
                        // page-table entry's address field are guaranteed aligned to
                        // 4KB by the MMU architecture.
                        let pml4 = &*(crate::mm::phys_to_virt(cr3 & addr_mask) as *const PageTable);
                        let pml4_idx = ((f.cr2 >> 39) & 0x1FF) as usize;
                        let pml4e = pml4.entry(pml4_idx);
                        s.puts("  PML4[");
                        s.dec(pml4_idx as u64);
                        s.puts("]=");
                        s.hex(pml4e);
                        s.puts("\n");
                        if pml4e & 1 != 0 {
                            let pdpt = &*(crate::mm::phys_to_virt(pml4e & addr_mask)
                                as *const PageTable);
                            let pdpt_idx = ((f.cr2 >> 30) & 0x1FF) as usize;
                            let pdpte = pdpt.entry(pdpt_idx);
                            s.puts("  PDPT[");
                            s.dec(pdpt_idx as u64);
                            s.puts("]=");
                            s.hex(pdpte);
                            s.puts("\n");
                            if pdpte & 1 != 0 && pdpte & (1 << 7) == 0 {
                                let pd = &*(crate::mm::phys_to_virt(pdpte & addr_mask)
                                    as *const PageTable);
                                ...
                                if pde & 1 != 0 && pde & (1 << 7) == 0 {
                                    let pt = &*(crate::mm::phys_to_virt(pde & addr_mask)
                                        as *const PageTable);
                                    ...
                                    s.hex(pte);
                                    s.puts("\n");
                                    // Scan all 512 PT entries to reveal the valid/zero pattern.
                                    // Tells us immediately if the PT was fully zeroed vs. partially populated.
                                    let mut valid_count: u32 = 0;
                                    let mut first_valid: i32 = -1;
                                    let mut last_valid: i32 = -1;
                                    for scan_i in 0..512usize {
                                        if pt.entry(scan_i) & 1 != 0 {
                                            valid_count += 1;
                                            if first_valid < 0 {
                                                first_valid = scan_i as i32;
                                            }
                                            last_valid = scan_i as i32;
                                        }
                                    }
                                    s.puts("  PT scan: valid=");
                                    s.dec(valid_count as u64);
                                    s.puts(" first=");
                                    if first_valid >= 0 {
                                        s.dec(first_valid as u64);
                                    } else {
                                        s.puts("none");
                                    }
                                    s.puts(" last=");
                                    if last_valid >= 0 {
                                        s.dec(last_valid as u64);
                                    } else {
                                        s.puts("none");
                                    }
                                    s.puts("\n");

                                    // Query frame allocator state for the PT frame itself.
                                    // is_free=true means the frame was returned to the
                                    // allocator (freed) — definitive double-alloc evidence.
                                    let pt_phys = pde & addr_mask;
                                    let (is_free, map_refs, _obj_refs, reclaimable) =
                                        crate::mm::query_frame_debug(pt_phys);
                                    s.puts("  PT frame ");
                                    s.hex(pt_phys);
                                    if is_free {
                                        s.puts(" FREE!");
                                    ...
                                    s.puts(" map_refs=");
                                    s.dec(map_refs as u64);
                                    s.puts(" recl=");
                                    s.dec(reclaimable as u64);
                                    s.puts("\n");

                                    // Sample first 3 valid entries to confirm ownership.
                                    // Sequential initrd pages (0x800000+) = procmgr's PT.
                                    let mut sample_valid = 0u32;
                                    for sample_i in 0..512usize {
                                        let sample_pte = pt.entry(sample_i);
                                        if sample_pte & 1 != 0 && sample_valid < 3 {
                                            s.puts("  PT[");
                                            s.dec(sample_i as u64);
                                            s.puts("] -> phys ");
                                            s.hex(sample_pte & addr_mask);
                                            s.puts("\n");
                                            sample_valid += 1;
                                        }
                                        if sample_valid >= 3 {
                                    ...
                                    // Dump ALL contiguous zero ranges in the PT.
                                    // Z[start..end] = run of zero entries (inclusive).
                                    // One large range = bulk memset; many small = individual unmaps.
                                    let mut in_zero_run = false;
                                    let mut zero_start = 0usize;
                                    for zi in 0..512usize {
                                        let zpte = pt.entry(zi);
                                        if zpte == 0 {
                                            if !in_zero_run {
                                                in_zero_run = true;
                                                zero_start = zi;
                                            ...
                                            s.puts("  Z[");
                                            s.dec(zero_start as u64);
                                            s.puts("..");
                                            s.dec((zi - 1) as u64);
                                            s.puts("] (");
                                            s.dec((zi - zero_start) as u64);
                                            s.puts(" entries)\n");
                                            in_zero_run = false;
                                        }
                                    }
